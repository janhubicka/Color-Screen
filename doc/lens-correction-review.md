...
| LS-007 | Fixed | A stalled non-Dufay automatic-registration pass may provisionally bypass the broad coverage gate, but only with the type-specific trusted-point minimum (30 for two-dimensional screens, 60 for vertical-strip screens) and only with the two lower-order free radial terms. Physical-envelope and profiled-Jacobian identifiability checks still apply. The provisional mapping is never final: newly discovered points must make the ordinary global coverage test pass, are pruned against that ordinary fit, and the final model is solved again. Dufay-like screens keep the temporary nonlinear bootstrap because their film geometry ultimately needs that model. |
