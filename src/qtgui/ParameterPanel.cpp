...
void ParameterPanel::addDoubleParameter(const QString &label, double min, double max, 
                            ...
                            std::function<bool(double)> validator)
{
    ...
    if (!quickSelects.empty()) {
        ...
        connect(combo, QOverload<int>::of(&QComboBox::activated), this, [combo, spin](int index){
    ...
    connect(spin, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, [this, setter](double val){
    ...
    if (validator || !specialValues.empty()) {
        m_widgetStateUpdaters.push_back([this, spin, combo, validator, specialValues](){
             ...
             for (auto const& [val, text] : specialValues) {
                 ...
                 if (val == 0.0 && text.contains("ICC")) {
                      if (!scan || scan->to_linear[0].empty()) enabled = false;
                 }
                 if (validator && !validator(val)) enabled = false;
...
void ParameterPanel::addSliderParameter(const QString &label, double min, double max, double scale, int decimals,
                            ...
                            std::function<bool(const ParameterState&)> enabledCheck)
{
    ...
    if (!suffix.isEmpty()) spin->setSuffix(QString(" %1").arg(suffix));
    if (!specialValueText.isEmpty()) spin->setSpecialValueText(specialValueText);
    ...
    auto sliderToValue = [min, max, scale, gamma, SLIDER_MAX](int s) -> double {
        if (gamma == 1.0) return (double)s / scale;
    ...
    auto valueToSlider = [min, max, scale, gamma, SLIDER_MAX](double v) -> int {
        if (gamma == 1.0) return qRound(v * scale);
    ...
    // Synchronization
    connect(slider, &QSlider::valueChanged, this, [this, spin, sliderToValue, setter](int val){
    ...
    connect(spin, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, [slider, valueToSlider](double val){
    ...
    // Updater: State -> UI
    m_paramUpdaters.push_back([slider, spin, getter, valueToSlider](const ParameterState &state){
        double val = getter(state);
        spin->blockSignals(true);
        spin->setValue(val);
        spin->blockSignals(false);
    ...
    // Enable Update
    if (enabledCheck) {
        m_widgetStateUpdaters.push_back([this, slider, spin, container, enabledCheck](){
            ParameterState state = m_stateGetter();
            ...
            spin->setEnabled(en);
             ...
             // Actually, disabling the container widget is enough if they were children.
             // But container is just a QWidget holding them. Yes.
             // Wait, m_form->addRow(label, container).
             // If I disable container, the label is NOT disabled.
             // I need to disable label.
...
void ParameterPanel::addEnumParameter(const QString &label, const std::map<int, QString> &options,
                          ...
                          std::function<bool(const ParameterState&)> enabledCheck)
{
    ...
    // Connect changes: UI -> State
    connect(combo, QOverload<int>::of(&QComboBox::activated), this, [this, combo, setter](int index){
...
QToolButton* ParameterPanel::addSeparator(const QString &title)
{
    ...
    group->setFlat(true);
    group->setStyleSheet("QGroupBox { border: none; margin: 0px; padding: 0px; }");
    ...
    arrowBtn->setStyleSheet("QToolButton { border: none; background: transparent; }");
