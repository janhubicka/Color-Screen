...
ParameterPanel::~ParameterPanel() {
  ...
  // been torn down. Detach our filters/callbacks while the target map is still
  // alive so a child's destroyed() signal cannot touch freed bookkeeping.
...
/** Discover standard numeric Reset rows and attach the common double-click
    gesture to their value editor and form label. */
void ParameterPanel::registerNumericDoubleClickResetTargets() {
  for (QToolButton *resetButton : findChildren<QToolButton *>()) {
    ...
    QDoubleSpinBox *spin =
        field ? field->findChild<QDoubleSpinBox *>() : nullptr;
    if (!field || !spin)
      continue; // Discrete Reset rows intentionally keep ordinary interaction.
...
void ParameterPanel::updateUI() {
  // Reset-enabled rows may be added late during derived-panel construction.
  // Discover them lazily so the gesture works without panel-specific wiring.
