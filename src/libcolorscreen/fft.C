...
template <typename T>
fft_plan<T>
fft_plan_r2c_2d (int n0, int n1, unsigned flags)
{
  ...
  T *in = (T *)fftw_malloc (sizeof (T) * n0 * n1);
  typename fft_complex_t<T>::type *out
      = (typename fft_complex_t<T>::type *)fftw_malloc (
...
template <typename T>
fft_plan<T>
fft_plan_c2r_2d (int n0, int n1, unsigned flags)
{
  std::lock_guard<std::mutex> lock (fft_lock);
  ...
  typename fft_complex_t<T>::type *in
      = (typename fft_complex_t<T>::type *)fftw_malloc (
          sizeof (typename fft_complex_t<T>::type) * n0 * (n1 / 2 + 1));
  T *out = (T *)fftw_malloc (sizeof (T) * n0 * n1);
  typename fft_plan_t<T>::type p;
  if constexpr (std::is_same_v<T, double>)
...
template <typename T>
fft_plan<T>
fft_plan_r2c_1d (int n, unsigned flags)
{
  std::lock_guard<std::mutex> lock (fft_lock);
...
template <typename T>
fft_plan<T>
fft_plan_r2r_1d (int n, fftw_r2r_kind kind, unsigned flags)
{
  std::lock_guard<std::mutex> lock (fft_lock);
  ...
  T *in = (T *)fftw_malloc (sizeof (T) * n);
  T *out = (T *)fftw_malloc (sizeof (T) * n);
  typename fft_plan_t<T>::type p;
  if constexpr (std::is_same_v<T, double>)
...
template fft_plan<double> fft_plan_r2c_2d<double> (int, int, unsigned);
template fft_plan<double> fft_plan_c2r_2d<double> (int, int, unsigned);
template fft_plan<double> fft_plan_r2c_1d<double> (int, unsigned);
template fft_plan<double> fft_plan_r2r_1d<double> (int, fftw_r2r_kind, unsigned);

template fft_plan<float> fft_plan_r2c_2d<float> (int, int, unsigned);
template fft_plan<float> fft_plan_c2r_2d<float> (int, int, unsigned);
template fft_plan<float> fft_plan_r2c_1d<float> (int, unsigned);
template fft_plan<float> fft_plan_r2r_1d<float> (int, fftw_r2r_kind, unsigned);
