...
template <typename T> static inline T my_abs(T x) { return x < 0 ? -x : x; }
...
public:
  ...
  rgbdata &demosaiced_data(int x, int y) {
  ...
  rgbdata &fast_demosaiced_data(int x, int y) {
    return m_demosaiced[y * m_area.width + x];
  };
  ...
  /* Determine the robust maximum value in the demosaiced image using a
     ...
     cancellation.  */
  luminosity_t find_robust_max(progress_info *progress) {
    histogram h;
    if (progress)
...
protected:
  ...
  always_inline_attr rgbdata &d(int x, int y) {
    x = std::clamp(x, 0, m_area.width - 1);
    y = std::clamp(y, 0, m_area.height - 1);
    return m_demosaiced[y * m_area.width + x];
  };
...
class demosaic_base : public demosaic_generic_base {
  ...
  inline pure_attr rgbdata nearest_demosaiced_interpolate(point_t scr) {
    point_t p = GEOMETRY::to_demosaiced_coordinates(scr);
  ...
  inline pure_attr rgbdata linear_demosaiced_interpolate(point_t scr) {
    point_t p = GEOMETRY::to_demosaiced_coordinates(scr);
    int sx, sy;
    coord_t rx = my_modf(p.x, &sx);
    coord_t ry = my_modf(p.y, &sy);
    ...
    if (sx >= 0 && sx < m_area.width - 1 && sy >= 0 && sy < m_area.height - 1) {
      rgbdata ret;
      ret.red = do_linear_interpolate(fast_demosaiced_data(sx, sy).red,
  ...
  inline pure_attr rgbdata bspline_demosaiced_interpolate(point_t scr) {
    point_t p = GEOMETRY::to_demosaiced_coordinates(scr);
    int sx, sy;
    coord_t rx = my_modf(p.x, &sx);
    coord_t ry = my_modf(p.y, &sy);
...
protected:
  bool initialize(ANALYZER *analysis) {
  ...
  always_inline_attr luminosity_t &dch(int x, int y, int ch) {
  ...
  always_inline_attr luminosity_t known(int x, int y) {
    int cx = std::clamp(x, 0, m_area.width - 1);
    int cy = std::clamp(y, 0, m_area.height - 1);
  ...
  bool generic_dominating_channel(progress_info *progress) {
...
#pragma omp parallel shared(progress, h, w) default(none)
    for (int y = 0; y < h; y++) {
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          ...
          bool left_g = GEOMETRY::demosaic_entry_color(x - 1, y) == ah_green;
          bool right_g = GEOMETRY::demosaic_entry_color(x + 1, y) == ah_green;
          bool up_g = GEOMETRY::demosaic_entry_color(x, y - 1) == ah_green;
          bool down_g = GEOMETRY::demosaic_entry_color(x, y + 1) == ah_green;
          ...
          if (h_green && v_green) {
            ...
            luminosity_t grad_h =
                my_abs(g_left - g_right) + my_abs(2 * known_val - kl2 - kr2);
            luminosity_t grad_v =
                my_abs(g_up - g_down) + my_abs(2 * known_val - ku2 - kd2);
            luminosity_t lapl_h =
            ...
            if (grad_h < grad_v)
              ...
              d(x, y)[(int)ah_green] =
                  (g_left + g_right + g_up + g_down) * (luminosity_t)0.25 +
            ...
            bool tl_g =
                GEOMETRY::demosaic_entry_color(x - 1, y - 1) == ah_green;
            bool tr_g =
                GEOMETRY::demosaic_entry_color(x + 1, y - 1) == ah_green;
            bool bl_g =
                GEOMETRY::demosaic_entry_color(x - 1, y + 1) == ah_green;
            ...
            if (count > 0) {
              ...
              luminosity_t kd2 = known(x, y + 2);
              luminosity_t lapl =
                  (4 * known_val - kl2 - kr2 - ku2 - kd2) * (luminosity_t)0.125;
  ...
  bool generic_interpolation_remaining_channels(progress_info *progress) {
    int w = m_area.width, h = m_area.height;
#pragma omp parallel shared(progress, h, w) default(none)
    for (int y = 0; y < h; y++) {
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          ...
          if (color != ah_red) {
            ...
            bool l_r = GEOMETRY::demosaic_entry_color(x - 1, y) == ah_red;
            bool r_r = GEOMETRY::demosaic_entry_color(x + 1, y) == ah_red;
            bool u_r = GEOMETRY::demosaic_entry_color(x, y - 1) == ah_red;
            bool d_r = GEOMETRY::demosaic_entry_color(x, y + 1) == ah_red;
            bool h_r = l_r && r_r;
            bool v_r = u_r && d_r;

            if (h_r && v_r) {
              luminosity_t rdl =
                  d(x - 1, y)[(int)ah_red] - d(x - 1, y)[(int)ah_green];
              luminosity_t rdr =
                  d(x + 1, y)[(int)ah_red] - d(x + 1, y)[(int)ah_green];
              luminosity_t rdu =
                  d(x, y - 1)[(int)ah_red] - d(x, y - 1)[(int)ah_green];
              luminosity_t rdd =
                  d(x, y + 1)[(int)ah_red] - d(x, y + 1)[(int)ah_green];
              luminosity_t rgh = my_abs(rdl - rdr);
              luminosity_t rgv = my_abs(rdu - rdd);
              if (rgh < rgv)
                d(x, y)[(int)ah_red] = g_here + (rdl + rdr) * (luminosity_t)0.5;
              else if (rgv < rgh)
                d(x, y)[(int)ah_red] = g_here + (rdu + rdd) * (luminosity_t)0.5;
              else
                d(x, y)[(int)ah_red] =
                    g_here + (rdl + rdr + rdu + rdd) * (luminosity_t)0.25;
            } else if (h_r) {
              luminosity_t rdl =
                  d(x - 1, y)[(int)ah_red] - d(x - 1, y)[(int)ah_green];
              luminosity_t rdr =
              ...
              bool tl_r =
                  GEOMETRY::demosaic_entry_color(x - 1, y - 1) == ah_red;
              bool tr_r =
                  GEOMETRY::demosaic_entry_color(x + 1, y - 1) == ah_red;
              bool bl_r =
                  GEOMETRY::demosaic_entry_color(x - 1, y + 1) == ah_red;
          ...
          if (color != ah_blue) {
            ...
            bool l_b = GEOMETRY::demosaic_entry_color(x - 1, y) == ah_blue;
            bool r_b = GEOMETRY::demosaic_entry_color(x + 1, y) == ah_blue;
            bool u_b = GEOMETRY::demosaic_entry_color(x, y - 1) == ah_blue;
            bool d_b = GEOMETRY::demosaic_entry_color(x, y + 1) == ah_blue;
            bool h_b = l_b && r_b;
            bool v_b = u_b && d_b;

            if (h_b && v_b) {
              luminosity_t bdl =
                  d(x - 1, y)[(int)ah_blue] - d(x - 1, y)[(int)ah_green];
              luminosity_t bdr =
                  d(x + 1, y)[(int)ah_blue] - d(x + 1, y)[(int)ah_green];
              luminosity_t bdu =
                  d(x, y - 1)[(int)ah_blue] - d(x, y - 1)[(int)ah_green];
              luminosity_t bdd =
                  d(x, y + 1)[(int)ah_blue] - d(x, y + 1)[(int)ah_green];
              ...
              if (bgh < bgv)
                ...
                d(x, y)[(int)ah_blue] =
                    g_here + (bdl + bdr + bdu + bdd) * (luminosity_t)0.25;
            } else if (h_b) {
              luminosity_t bdl =
                  d(x - 1, y)[(int)ah_blue] - d(x - 1, y)[(int)ah_green];
              luminosity_t bdr =
                  d(x + 1, y)[(int)ah_blue] - d(x + 1, y)[(int)ah_green];
              d(x, y)[(int)ah_blue] = g_here + (bdl + bdr) * (luminosity_t)0.5;
            } else if (v_b) {
              luminosity_t bdu =
                  d(x, y - 1)[(int)ah_blue] - d(x, y - 1)[(int)ah_green];
              luminosity_t bdd =
                  d(x, y + 1)[(int)ah_blue] - d(x, y + 1)[(int)ah_green];
              d(x, y)[(int)ah_blue] = g_here + (bdu + bdd) * (luminosity_t)0.5;
            } else {
              /* Check diagonals for blue.  */
              bool tl_b =
                  GEOMETRY::demosaic_entry_color(x - 1, y - 1) == ah_blue;
              bool tr_b =
                  GEOMETRY::demosaic_entry_color(x + 1, y - 1) == ah_blue;
              bool bl_b =
                  GEOMETRY::demosaic_entry_color(x - 1, y + 1) == ah_blue;
  ...
  hamilton_adams_interpolation_dominating_channel(progress_info *progress) {
    ...
    if (progress)
      progress->set_task("demosaicing dominating channel (hamilton-adams)", h);
    ...
    for (int y = 0; y < h; y++) {
      ...
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          ...
          pbh = bh;
          luminosity_t bv =
              (pbv[x] + c_20 - d(x - 2, y)[ah_green]) * (luminosity_t)0.5;
          pbv[x] = bv;
...
#pragma omp parallel shared(progress, h, w, predA) default(none)
      for (int y = 1; y < h - 1; y++) {
        if (!progress || !progress->cancel_requested())
          for (int x = 1; x < w - 1; x++) {
            int color = GEOMETRY::demosaic_entry_color(x, y);
            if (color != ah_green || (int)predA.test_bit(x - 1, y) +
  ...
  always_inline_attr void hamilton_adams_interpolate_color(luminosity_t g_here,
                                                           int x, int y) {
    /* Check cardinal neighbors for red.  */
    ...
    if (h_r) {
      ...
      if (dp < dn)
        ...
        d(x, y)[(int)ah_chn] =
            (r_1_1 + r11 + r_11 + r1_1 + 4 * g00 - g_1_1 - g11 - g_11 - g1_1) *
            (luminosity_t)0.25;
    }
  };
...
#pragma omp parallel shared(progress, h, w) default(none)
  ...
  luminosity_t interp(int x, int y, int ch,
                      const std::vector<luminosity_t> &G_dir) {
    int w = m_area.width, h = m_area.height;
    x = std::clamp(x, 0, w - 1);
    y = std::clamp(y, 0, h - 1);
    ...
    if (l_c && r_c) {
      ...
      return g_here + (dch(x - 1, y, ch) - G_dir[y * w + xl] +
                       dch(x + 1, y, ch) - G_dir[y * w + xr]) *
                          (luminosity_t)0.5;
    } else if (u_c && d_c) {
      int yu = std::clamp(y - 1, 0, h - 1);
      int yd = std::clamp(y + 1, 0, h - 1);
      return g_here + (dch(x, y - 1, ch) - G_dir[yu * w + x] +
  ...
  bool ahd_interpolation_dominating_channel(progress_info *progress) {
    int w = m_area.width, h = m_area.height;
    if (progress)
    ...
    for (int y = 0; y < h; y++) {
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          ...
          green_h[y * w + x] = (g_10 + g10) * (luminosity_t)0.5 +
                               (2 * c00 - c_20 - c20) * (luminosity_t)0.25;
          green_v[y * w + x] = (g0_1 + g01) * (luminosity_t)0.5 +
                               (2 * c00 - c0_2 - c02) * (luminosity_t)0.25;
        }
      if (progress)
        progress->inc_progress();
...
#pragma omp parallel shared(progress, h, w, predA, green_h, green_v, h_scores, \
                                v_scores) default(none)
    for (int y = 0; y < h; y++) {
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          int color = GEOMETRY::demosaic_entry_color(x, y);
...
#pragma omp parallel shared(progress, h, w, predA) default(none)
  ...
  template <int ah_green, int ah_red, int ah_blue>
  bool ahd_interpolation_remaining_channels(progress_info *progress) {
    int h = m_area.height, w = m_area.width;
    if (progress)
...
#pragma omp parallel shared(progress, h, w) default(none)
  ...
  static always_inline_attr luminosity_t median3(luminosity_t a, luminosity_t b,
                                                 luminosity_t c) {
  ...
  /* Hamilton-Adams green estimate from one cardinal direction.
     ...
     support.  */
  static always_inline_attr luminosity_t ha_green_one_direction(
      luminosity_t g_neighbor, luminosity_t c_here, luminosity_t c_far) {
  ...
  bool amaze_interpolation(progress_info *progress) {
    int w = m_area.width, h = m_area.height;
    luminosity_t range = find_robust_max(progress);
    if (progress && progress->cancelled())
      return false;
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = 0; y < h; y++) {
      ...
      for (int x = 0; x < w; x++) {
        ...
        luminosity_t cru =
            cu1 * (wt_v(x, y - 2) + wt_v(x, y)) /
            (wt_v(x, y - 2) * (eps + c00) + wt_v(x, y) * (eps + cu2));
        luminosity_t crd =
            cd1 * (wt_v(x, y + 2) + wt_v(x, y)) /
            (wt_v(x, y + 2) * (eps + c00) + wt_v(x, y) * (eps + cd2));
        luminosity_t crl =
            cl1 * (wt_h(x - 2, y) + wt_h(x, y)) /
            (wt_h(x - 2, y) * (eps + c00) + wt_h(x, y) * (eps + cl2));
        luminosity_t crr =
            cr1 * (wt_h(x + 2, y) + wt_h(x, y)) /
            (wt_h(x + 2, y) * (eps + c00) + wt_h(x, y) * (eps + cr2));
        ...
        if (c00 > clip_pt8 || g_int_v_ha > clip_pt8 || g_int_h_ha > clip_pt8) {
          guar = guha;
          gdar = gdha;
          glar = glha;
          grar = grha;
        ...
        if (is_green) {
          ...
          if (hcd_val > 0) {
            ...
            if (3 * hcd_val > (g_int_h + c00))
              ...
              luminosity_t w2 = 1 - 3 * hcd_val / (eps + g_int_h + c00);
        ...
        if (is_green) {
          ...
          if (vcd_val > 0) {
            ...
            if (3 * vcd_val > (g_int_v + c00))
              ...
              luminosity_t w2 = 1 - 3 * vcd_val / (eps + g_int_v + c00);
    ...
    for (int y = 0; y < h; y++) {
      ...
      for (int x = 0; x < w; x++) {
        ...
        auto vcd_at = [&](int xx, int yy) -> luminosity_t {
          return vcd[std::clamp(yy, 0, h - 1) * w + std::clamp(xx, 0, w - 1)];
        };
        auto hcd_at = [&](int xx, int yy) -> luminosity_t {
          return hcd[std::clamp(yy, 0, h - 1) * w + std::clamp(xx, 0, w - 1)];
        };
        ...
        luminosity_t uave =
            vcd0 + vcd_at(x, y - 1) + vcd_at(x, y - 2) + vcd_at(x, y - 3);
        luminosity_t dave =
            vcd0 + vcd_at(x, y + 1) + vcd_at(x, y + 2) + vcd_at(x, y + 3);
        ...
        luminosity_t lave =
            hcd0 + hcd_at(x - 1, y) + hcd_at(x - 2, y) + hcd_at(x - 3, y);
        luminosity_t rave =
            hcd0 + hcd_at(x + 1, y) + hcd_at(x + 2, y) + hcd_at(x + 3, y);
        ...
        luminosity_t vcd_var =
            epssq + wt_v_val * dgrb_v_var_d + (1 - wt_v_val) * dgrb_v_var_u;
        luminosity_t hcd_var =
            epssq + wt_h_val * dgrb_h_var_r + (1 - wt_h_val) * dgrb_h_var_l;
        ...
        if (((luminosity_t)0.5 - var_wt) * ((luminosity_t)0.5 - diff_wt) > 0 &&
...
#pragma omp parallel for schedule(dynamic, 16)
      for (int y = 0; y < h; y++) {
        ...
        for (int x = 0; x < w; x++) {
          ...
          luminosity_t val =
              gauss_odd[0] * cdq(x, y) +
              gauss_odd[1] * (cdq(x - 1, y + 1) + cdq(x + 1, y - 1) +
                              cdq(x + 1, y + 1) + cdq(x - 1, y - 1)) +
              gauss_odd[2] * (cdq(x, y - 2) + cdq(x - 2, y) + cdq(x + 2, y) +
                              cdq(x, y + 2)) +
              gauss_odd[3] * (cdq(x - 2, y + 2) + cdq(x + 2, y - 2) +
                              cdq(x - 2, y - 2) + cdq(x + 2, y + 2));

          luminosity_t grad_val =
              gauss_grad[0] * dhv(x, y) +
              gauss_grad[1] * (dhv(x, y - 1) + dhv(x - 1, y) + dhv(x + 1, y) +
                               dhv(x, y + 1)) +
              gauss_grad[2] * (dhv(x - 1, y - 1) + dhv(x + 1, y - 1) +
                               dhv(x - 1, y + 1) + dhv(x + 1, y + 1)) +
              gauss_grad[3] * (dhv(x, y - 2) + dhv(x - 2, y) + dhv(x + 2, y) +
                               dhv(x, y + 2)) +
              gauss_grad[4] *
                  (dhv(x - 1, y - 2) + dhv(x + 1, y - 2) + dhv(x - 2, y - 1) +
                   dhv(x + 2, y - 1) + dhv(x - 2, y + 1) + dhv(x + 2, y + 1) +
                   dhv(x - 1, y + 2) + dhv(x + 1, y + 2)) +
              gauss_grad[5] * (dhv(x - 2, y - 2) + dhv(x + 2, y - 2) +
        ...
        for (int x = 3; x < w - 3; x++) {
          ...
          for (int dy = -6; dy <= 6; dy += 2)
            for (int dx = -6; dx <= 6; dx += 2) {
              ...
              sumh += nh;
              sumv += nv;
              sumsqh += (cf - known(nx - 1, ny)) * (cf - known(nx - 1, ny)) +
                        (cf - known(nx + 1, ny)) * (cf - known(nx + 1, ny));
              sumsqv += (cf - known(nx, ny - 1)) * (cf - known(nx, ny - 1)) +
                        (cf - known(nx, ny + 1)) * (cf - known(nx, ny + 1));
              area_wt += 1;
            }
    ...
    for (int y = 0; y < h; y++) {
      ...
      for (int x = 0; x < w; x++) {
        ...
        auto hw = [&](int xx, int yy) -> luminosity_t {
          return hv_wt[std::clamp(yy, 0, h - 1) * w + std::clamp(xx, 0, w - 1)];
        };
        ...
        luminosity_t dgrb = hv_wt[idx] * vcd[idx] + (1 - hv_wt[idx]) * hcd[idx];
...
#pragma omp parallel for schedule(dynamic, 16)
  ...
  template <int ah_green, int ah_red, int ah_blue>
  bool rcd_interpolation(progress_info *progress) {
    int w = m_area.width, h = m_area.height;
      ...
      for (int y = 4; y < h - 4; y++) {
        ...
        for (int x = 4; x < w - 4; x++) {
          /* Accumulate 3-row vertical and 3-column horizontal
             statistics.  */
          luminosity_t v_stat =
              std::max(epssq, v_sq_row_0[x] + v_sq_row_1[x] + v_sq_row_2[x]);
          luminosity_t h_stat =
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = 2; y < h - 2; y++) {
      ...
      for (int x = 2; x < w - 2; x++) {
        ...
        lpf[y * w + x] =
            known(x, y) +
            (luminosity_t)0.5 * (known(x, y - 1) + known(x, y + 1) +
                                 known(x - 1, y) + known(x + 1, y)) +
            (luminosity_t)0.25 * (known(x - 1, y - 1) + known(x + 1, y - 1) +
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = 4; y < h - 4; y++) {
      ...
      for (int x = 4; x < w - 4; x++) {
        ...
        luminosity_t n_grad = eps + my_abs(known(x, y - 1) - known(x, y + 1)) +
                              my_abs(cfa_i - known(x, y - 2)) +
                              my_abs(known(x, y - 1) - known(x, y - 3)) +
                              my_abs(known(x, y - 2) - known(x, y - 4));
        luminosity_t s_grad = eps + my_abs(known(x, y + 1) - known(x, y - 1)) +
                              my_abs(cfa_i - known(x, y + 2)) +
                              my_abs(known(x, y + 1) - known(x, y + 3)) +
                              my_abs(known(x, y + 2) - known(x, y + 4));
        luminosity_t w_grad = eps + my_abs(known(x - 1, y) - known(x + 1, y)) +
                              my_abs(cfa_i - known(x - 2, y)) +
                              my_abs(known(x - 1, y) - known(x - 3, y)) +
                              my_abs(known(x - 2, y) - known(x - 4, y));
        luminosity_t e_grad = eps + my_abs(known(x + 1, y) - known(x - 1, y)) +
                              my_abs(cfa_i - known(x + 2, y)) +
                              my_abs(known(x + 1, y) - known(x + 3, y)) +
                              my_abs(known(x + 2, y) - known(x + 4, y));
        ...
        luminosity_t n_est =
            known(x, y - 1) * lpf2 / (eps + lpfi + lpf[(y - 2) * w + x]);
        luminosity_t s_est =
            known(x, y + 1) * lpf2 / (eps + lpfi + lpf[(y + 2) * w + x]);
        luminosity_t w_est =
            known(x - 1, y) * lpf2 / (eps + lpfi + lpf[y * w + (x - 2)]);
        luminosity_t e_est =
            known(x + 1, y) * lpf2 / (eps + lpfi + lpf[y * w + (x + 2)]);
        ...
        luminosity_t vh_neighbourhood =
            ...
            (vh_dir[(y - 1) * w + (x - 1)] + vh_dir[(y - 1) * w + (x + 1)] +
             vh_dir[(y + 1) * w + (x - 1)] + vh_dir[(y + 1) * w + (x + 1)]);
        luminosity_t vh_disc = (my_abs((luminosity_t)0.5 - vh_central) <
...
#pragma omp parallel for schedule(dynamic, 16)
      for (int y = 3; y < h - 3; y++) {
        ...
        for (int x = 3; x < w - 3; x++) {
          ...
          luminosity_t p = (known(x - 3, y - 3) - known(x - 1, y - 1) -
          ...
          luminosity_t q = (known(x + 3, y - 3) - known(x + 1, y - 1) -
                            known(x - 1, y + 1) + known(x - 3, y + 3)) -
                           3 * (known(x + 2, y - 2) + known(x - 2, y + 2)) +
                           6 * known(x, y);
    ...
    for (int y = 4; y < h - 4; y++) {
      ...
      for (int x = 4; x < w - 4; x++) {
        ...
        luminosity_t pq_neighbourhood =
            (luminosity_t)0.25 *
            (pq_dir[(y - 1) * w + (x - 1)] + pq_dir[(y - 1) * w + (x + 1)] +
             pq_dir[(y + 1) * w + (x - 1)] + pq_dir[(y + 1) * w + (x + 1)]);
        luminosity_t pq_disc = (my_abs((luminosity_t)0.5 - pq_central) <
        ...
        d(x, y)[(int)c] = g_here + pq_disc * q_est + (1 - pq_disc) * p_est;
      }
      if (progress)
        progress->inc_progress();
    }
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = 4; y < h - 4; y++) {
      ...
      for (int x = 4; x < w - 4; x++) {
        ...
        luminosity_t n_1 = eps + my_abs(g_here - d(x, y - 2)[(int)ah_green]);
        luminosity_t s_1 = eps + my_abs(g_here - d(x, y + 2)[(int)ah_green]);
        luminosity_t w_1 = eps + my_abs(g_here - d(x - 2, y)[(int)ah_green]);
        luminosity_t e_1 = eps + my_abs(g_here - d(x + 2, y)[(int)ah_green]);
        ...
        for (int c = ah_red; c <= ah_blue; c += (ah_blue - ah_red)) {
          ...
          luminosity_t sn_abs = fabs(dch(x, y - 1, c) - dch(x, y + 1, c));
          luminosity_t ew_abs = fabs(dch(x - 1, y, c) - dch(x + 1, y, c));

          /* Cardinal gradients.  */
          luminosity_t n_grad =
              n_1 + sn_abs + fabs(dch(x, y - 1, c) - dch(x, y - 3, c));
          luminosity_t s_grad =
              s_1 + sn_abs + fabs(dch(x, y + 1, c) - dch(x, y + 3, c));
          luminosity_t w_grad =
              w_1 + ew_abs + fabs(dch(x - 1, y, c) - dch(x - 3, y, c));
          luminosity_t e_grad =
              e_1 + ew_abs + fabs(dch(x + 1, y, c) - dch(x + 3, y, c));
          ...
          d(x, y)[(int)c] = g_here + vh_disc * h_est + (1 - vh_disc) * v_est;
        }
      }
      if (progress)
        progress->inc_progress();
  ...
  */
  /**
   * RCD (Ratio-Corrected Demosaicing) specialized for 4x4 sparse screen
   * patterns.
   *
   ...
   */
  template <int ah_green, int ah_red, int ah_blue>
  bool rcd_interpolation_4x4(progress_info *progress) {
    int w = m_area.width, h = m_area.height;
    constexpr luminosity_t eps = (luminosity_t)1e-5;
    constexpr luminosity_t epssq = (luminosity_t)1e-10;
      ...
      for (int y = 4; y < h - 4; y++) {
        ...
        for (int x = 4; x < w - 4; x++) {
          luminosity_t v = (known(x, y + 1 - 3) - known(x, y + 1 - 1) -
                           ...
                           6 * known(x, y + 1);
          v_sq_row_2[x] = (GEOMETRY::demosaic_entry_color(x, y + 1) == ah_red)
                              ? (v * v)
                              : 0;
        }
        std::vector<luminosity_t> h_sq(w, 0);
        for (int x = 3; x < w - 3; x++) {
          luminosity_t hv = (known(x - 3, y) - known(x - 1, y) -
                            ...
                            6 * known(x, y);
          h_sq[x] =
              (GEOMETRY::demosaic_entry_color(x, y) == ah_red) ? (hv * hv) : 0;
        }
        for (int x = 4; x < w - 4; x++) {
          luminosity_t v_stat = std::max(
              epssq, v_sq_row_0[x] + 2 * v_sq_row_1[x] + v_sq_row_2[x]);
          luminosity_t h_stat =
              std::max(epssq, h_sq[x - 1] + 2 * h_sq[x] + h_sq[x + 1]);
          vh_dir[y * w + x] = v_stat / (v_stat + h_stat);
        }
        std::swap(v_sq_row_0, v_sq_row_1);
        std::swap(v_sq_row_1, v_sq_row_2);
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = border; y < h - border; y++) {
      ...
      for (int x = border; x < w - border; x++) {
        ...
        luminosity_t lpf2 = lpfi + lpfi;
        luminosity_t n_est =
            known(x, y - 1) * lpf2 / (eps + lpfi + lpf[(y - 4) * w + x]);
        luminosity_t s_est =
            known(x, y + 1) * lpf2 / (eps + lpfi + lpf[(y + 4) * w + x]);
        luminosity_t w_est =
            known(x - 1, y) * lpf2 / (eps + lpfi + lpf[y * w + (x - 4)]);
        luminosity_t e_est =
            known(x + 1, y) * lpf2 / (eps + lpfi + lpf[y * w + (x + 4)]);
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = border; y < h - border; y++) {
      ...
      for (int x = border; x < w - border; x++) {
        ...
        luminosity_t nw_grad =
            eps + my_abs(g_here - d(x - 2, y - 2)[(int)ah_green]);
        luminosity_t se_grad =
            eps + my_abs(g_here - d(x + 2, y + 2)[(int)ah_green]);
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = border; y < h - border; y++) {
      ...
      for (int x = border; x < w - border; x++) {
        ...
        luminosity_t cd_g_h =
            dch(gx_h, gy_h, ah_red) - d(gx_h, gy_h)[(int)ah_green];
        luminosity_t cd_g_v =
            dch(gx_v, gy_v, ah_red) - d(gx_v, gy_v)[(int)ah_green];
        luminosity_t cd_b_h =
            dch(bx_h, by_h, ah_blue) - d(bx_h, by_h)[(int)ah_green];
        luminosity_t cd_b_v =
            dch(bx_v, by_v, ah_blue) - d(bx_v, by_v)[(int)ah_green];

        /* Gradients of the dominating channel to the neighbours. */
        luminosity_t grad_g_h =
            eps + my_abs(r_here - d(gx_h, gy_h)[(int)ah_green]);
        luminosity_t grad_g_v =
            eps + my_abs(r_here - d(gx_v, gy_v)[(int)ah_green]);
        luminosity_t grad_b_h =
            eps + my_abs(r_here - d(bx_h, by_h)[(int)ah_green]);
        luminosity_t grad_b_v =
            eps + my_abs(r_here - d(bx_v, by_v)[(int)ah_green]);

        /* VH blend combined with gradient steering.
           vh≈1 → prefer H, vh≈0 → prefer V.
           Gradient steering handles diagonal edges where H and V cross the
           edge. */
        luminosity_t w_g_h = vh / grad_g_h;
        luminosity_t w_g_v = (1.0 - vh) / grad_g_v;
        d(x, y)[(int)ah_red] =
            r_here + (w_g_h * cd_g_h + w_g_v * cd_g_v) / (w_g_h + w_g_v + eps);

        luminosity_t w_b_h = vh / grad_b_h;
        luminosity_t w_b_v = (1.0 - vh) / grad_b_v;
        d(x, y)[(int)ah_blue] =
            r_here + (w_b_h * cd_b_h + w_b_v * cd_b_v) / (w_b_h + w_b_v + eps);
      }
      if (progress)
        progress->inc_progress();
    }
  ...
  template <int ah_green, int ah_red, int ah_blue>
  bool lmmse_interpolation(progress_info *progress) {
    int w = m_area.width, h = m_area.height;
    ...
    for (int y = 2; y < h - 2; y++) {
      ...
      for (int x = 2; x < w - 2; x++) {
        ...
        luminosity_t vd = (luminosity_t)(-0.25) *
      ...
      for (int x = 2; x < w - 2; x++) {
        ...
        luminosity_t hd = (luminosity_t)0.25 * (cfa[idx - 2] + cfa[idx + 2]) -
                          (luminosity_t)0.5 * (cfa[idx - 1] + c + cfa[idx + 1]);
        luminosity_t vd =
            (luminosity_t)0.25 * (cfa[(y - 2) * w + x] + cfa[(y + 2) * w + x]) -
            (luminosity_t)0.5 *
                (cfa[(y - 1) * w + x] + c + cfa[(y + 1) * w + x]);
...
#pragma omp parallel for schedule(dynamic, 16)
    for (int y = 4; y < h - 4; y++) {
      ...
      for (int x = 4; x < w - 4; x++) {
        ...
        luminosity_t mu =
            ...
            (luminosity_t)9;
        luminosity_t vx = (luminosity_t)1e-7 + (p_1 - mu) * (p_1 - mu) +
    ...
    for (int y = 1; y < h - 1; y++) {
      ...
      for (int x = 1; x < w - 1; x++) {
        ...
        rgb[c_other][idx] =
            g + (luminosity_t)0.25 *
                    (rgb[c_other][(y - 1) * w + x] -
                     rgb[ah_green][(y - 1) * w + x] + rgb[c_other][idx - 1] -
                     rgb[ah_green][idx - 1] + rgb[c_other][idx + 1] -
                     rgb[ah_green][idx + 1] + rgb[c_other][(y + 1) * w + x] -
      ...
      for (int y = 1; y < h - 1; y++) {
        for (int x = 1; x < w - 1; x++) {
          ...
          if (c == ah_green) {
            ...
            rgb[ah_green][idx] =
                (luminosity_t)0.5 * (rgb[ah_red][idx] - corr_r[idx] +
    ...
    for (int step = 0; step < 0; step++) {
      ...
      for (int y = 2; y < h - 2; y++) {
        ...
        for (int x = 2; x < w - 2; x++) {
          ...
          int idx = y * w + x;
          luminosity_t dL =
              1 / (1 + my_abs(rgb[c][idx - 2] - rgb[c][idx]) +
                   my_abs(rgb[ah_green][idx + 1] - rgb[ah_green][idx - 1]));
          luminosity_t dR =
              1 / (1 + my_abs(rgb[c][idx + 2] - rgb[c][idx]) +
                   my_abs(rgb[ah_green][idx + 1] - rgb[ah_green][idx - 1]));
          luminosity_t dU =
              1 / (1 + my_abs(rgb[c][idx - 2 * w] - rgb[c][idx]) +
                   my_abs(rgb[ah_green][idx + w] - rgb[ah_green][idx - w]));
          luminosity_t dD =
              1 / (1 + my_abs(rgb[c][idx + 2 * w] - rgb[c][idx]) +
                   my_abs(rgb[ah_green][idx + w] - rgb[ah_green][idx - w]));
          rgb[ah_green][idx] =
              rgb[c][idx] + ((rgb[ah_green][idx - 1] - rgb[c][idx - 1]) * dL +
        ...
        for (int x = 2; x < w - 2; x++) {
          ...
          for (int cc = 0; cc < 3; cc++) {
            if (cc == ah_green)
              continue;
            luminosity_t dL =
                1 / (1 + my_abs(rgb[ah_green][idx - 2] - rgb[ah_green][idx]) +
        ...
        for (int x = 2; x < w - 2; x++) {
          ...
          rgb[c_other][idx] =
              ...
              ((rgb[ah_green][idx - 1] - rgb[c_other][idx - 1]) * d_l +
               (rgb[ah_green][idx + 1] - rgb[c_other][idx + 1]) * d_r +
               (rgb[ah_green][idx - w] - rgb[c_other][idx - w]) * d_u +
               (rgb[ah_green][idx + w] - rgb[c_other][idx + w]) * d_d) /
                  (d_l + d_r + d_u + d_d);
        }
      }
      if (progress)
    ...
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        int idx = y * w + x;
        d(x, y)[(int)ah_red] =
            std::max((luminosity_t)0, range * gamma_inv(rgb[ah_red][idx]));
        d(x, y)[(int)ah_green] =
            std::max((luminosity_t)0, range * gamma_inv(rgb[ah_green][idx]));
        d(x, y)[(int)ah_blue] =
            std::max((luminosity_t)0, range * gamma_inv(rgb[ah_blue][idx]));
      }
      if (progress)
        progress->inc_progress();
    }
  ...
  interp_dcraw(int x, int y, int ch, const std::vector<luminosity_t> &G_dir,
    ...
    if (l_c && r_c) {
      int xl = std::clamp(x - 1, 0, w - 1);
      int xr = std::clamp(x + 1, 0, w - 1);
      val = g_here + (dch(x - 1, y, ch) - G_dir[y * w + xl] +
                      dch(x + 1, y, ch) - G_dir[y * w + xr]) *
                         (luminosity_t)0.5;
    } else if (u_c && d_c) {
      int yu = std::clamp(y - 1, 0, h - 1);
      int yd = std::clamp(y + 1, 0, h - 1);
      val = g_here + (dch(x, y - 1, ch) - G_dir[yu * w + x] +
                      dch(x, y + 1, ch) - G_dir[yd * w + x]) *
                         (luminosity_t)0.5;
    } else if (l_c)
  ...
  /* Adaptive Homogeneity-Directed (AHD) demosaicing algorithm (dcraw variant).
     ...
     AH_BLUE is the blue channel index.  */
  template <int ah_green, int ah_red, int ah_blue>
  bool ahd_interpolation_dominating_channel_dcraw(progress_info *progress) {
    int w = m_area.width, h = m_area.height;
    if (progress)
      ...
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          ...
          luminosity_t gh = (g_10 + g10) * (luminosity_t)0.5 +
                            (2 * c00 - c_20 - c20) * (luminosity_t)0.25;
          green_h[y * w + x] =
              std::clamp(gh, std::min(g_10, g10), std::max(g_10, g10));

          luminosity_t gv = (g0_1 + g01) * (luminosity_t)0.5 +
                            (2 * c00 - c0_2 - c02) * (luminosity_t)0.25;
          green_v[y * w + x] =
              std::clamp(gv, std::min(g0_1, g01), std::max(g0_1, g01));
        }
      if (progress)
        progress->inc_progress();
    }
...
#pragma omp parallel shared(progress, h, w, range, rgb_h, rgb_v, lab_h, lab_v, \
                                green_h, green_v, srgb_white) default(none)
    for (int y = 0; y < h; y++) {
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          int idx = y * w + x;
        ...
        for (int x = 0; x < w; x++) {
          ...
          for (int d = 0; d < 2; d++) {
            ...
            for (int i = 0; i < 4; i++) {
              ...
              ldiff[d][i] =
                  my_abs((int)(center.l * 100.0f) - (int)(neigh.l * 100.0f));
              abdiff[d][i] =
                  ((int)(center.a * 100.0f) - (int)(neigh.a * 100.0f)) *
                      ((int)(center.a * 100.0f) - (int)(neigh.a * 100.0f)) +
                  ((int)(center.b * 100.0f) - (int)(neigh.b * 100.0f)) *
                      ((int)(center.b * 100.0f) - (int)(neigh.b * 100.0f));
            }
          }
...
#pragma omp parallel shared(progress, h, w, rgb_h, rgb_v, homo_h,              \
      ...
      if (!progress || !progress->cancel_requested())
        for (int x = 0; x < w; x++) {
          ...
          if (hm_v > hm_h) {
            ...
            d(x, y)[ah_green] =
                (rgb_h[y * w + x][ah_green] + rgb_v[y * w + x][ah_green]) *
                (luminosity_t)0.5;
            d(x, y)[ah_red] =
  ...
  /* Step 2 of the AHD algorithm (dcraw variant): interpolate red and blue
     ...
     AH_BLUE is the blue channel.  */
  template <int ah_green, int ah_red, int ah_blue>
  bool ahd_interpolation_remaining_channels_dcraw(progress_info *progress) {
    return !progress || !progress->cancelled();
  }
};
...
public:
  bool demosaic(ANALYZER *analyze, render *r,
    ...
    case render_parameters::ahd_demosaic:
      ...
      // return false;
      if (!this->template ahd_interpolation_dominating_channel_dcraw<
              base_geometry::blue, base_geometry::red, base_geometry::green>(
              progress))
        return false;
      if (!this->template ahd_interpolation_remaining_channels<
              base_geometry::blue, base_geometry::red, base_geometry::green>(
              progress))
        return false;
      break;
    case render_parameters::amaze_demosaic:
      if (!this->template amaze_interpolation<base_geometry::blue,
                                              base_geometry::red,
                                              base_geometry::green, true>(
              progress))
        return false;
      break;
    case render_parameters::rcd_demosaic:
      if (!this->template rcd_interpolation<
              base_geometry::blue, base_geometry::red, base_geometry::green>(
              progress))
        return false;
      break;
    case render_parameters::lmmse_demosaic:
      if (!this->template lmmse_interpolation<
              base_geometry::blue, base_geometry::red, base_geometry::green>(
              progress))
        return false;
      break;
