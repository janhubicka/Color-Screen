...
public:
  ...
  bool
  save_tiff (const char *filename, progress_info *progress)
  {
  ...
  hamilton_adams_interpolate_color (luminosity_t g_here, int x, int y)
  {
    ...
    if (h_r)
    ...
    else
      {
        ...
        luminosity_t dn
            = my_abs (2 * g00 - g_1_1 - g11) + my_abs (r_1_1 - r11);
        luminosity_t dp
            = my_abs (2 * g00 - g_11 - g1_1) + my_abs (r_11 - r1_1);
        if (dn < dp)
          d (x, y)[(int)ah_chn]
        ...
        for (int x = 4; x < w - 4; x++)
          {
            ...
            luminosity_t n_grad
                = eps + my_abs (known (x, y - 1) - known (x, y + 1))
            ...
            luminosity_t s_grad
                = eps + my_abs (known (x, y + 1) - known (x, y - 1))
                  ...
                  + my_abs (known (x, y + 2) - known (x, y + 4));
            luminosity_t w_grad
                = eps + my_abs (known (x - 1, y) - known (x + 1, y))
            ...
            for (int i = -3; i <= 3; i++)
              for (int j = -3; j <= 3; j++)
                if (GEOMETRY::demosaic_entry_color (x + j, y + i) == ah_green)
                  {
                    luminosity_t gw
                        = std::exp (-(j * j + i * i) / (luminosity_t)8);
        ...
        for (int x = border; x < w - border; x++)
          {
            ...
            luminosity_t lpf2 = lpfi + lpfi;
            luminosity_t n_est = known (x, y - 1) * lpf2
                                 / (eps + lpfi + lpf[(y - 4) * w + x]);
            luminosity_t s_est = known (x, y + 1) * lpf2
                                 / (eps + lpfi + lpf[(y + 4) * w + x]);
            luminosity_t w_est = known (x - 1, y) * lpf2
                                 / (eps + lpfi + lpf[y * w + (x - 4)]);
            luminosity_t e_est = known (x + 1, y) * lpf2
                                 / (eps + lpfi + lpf[y * w + (x + 4)]);
            ...
            luminosity_t vh_central = vh_dir[idx];
            luminosity_t vh_nbh = (luminosity_t)0.25
                                  * (vh_dir[(y - 1) * w + (x - 1)]
                                     + vh_dir[(y - 1) * w + (x + 1)]
                                     + vh_dir[(y + 1) * w + (x - 1)]
                                     + vh_dir[(y + 1) * w + (x + 1)]);
        ...
        for (int x = border; x < w - border; x++)
          {
            ...
            d (x, y)[(int)ah_red]
                = r_here
                  + (w_g_h * cd_g_h + w_g_v * cd_g_v) / (w_g_h + w_g_v + eps);

            luminosity_t w_b_h = vh / grad_b_h;
            luminosity_t w_b_v = (1.0 - vh) / grad_b_v;
            d (x, y)[(int)ah_blue]
                = r_here
                  + (w_b_h * cd_b_h + w_b_v * cd_b_v) / (w_b_h + w_b_v + eps);
          }
        if (progress)
          progress->inc_progress ();
      }
            ...
            denoise_parameters denoise_params, progress_info *progress)
  {
    ...
    switch (alg)
      ...
      case render_parameters::rcd_demosaic:
        if (!this->template rcd_interpolation<base_geometry::blue,
                                              base_geometry::red,
                                              base_geometry::green> (progress))
          return false;
        break;
      case render_parameters::lmmse_demosaic:
        if (!this->template lmmse_interpolation<base_geometry::blue,
                                                base_geometry::red,
                                                base_geometry::green> (
                progress))
          return false;
...
public:
            ...
            denoise_parameters denoise_params, progress_info *progress)
  {
    ...
    switch (alg)
      ...
      case render_parameters::rcd_demosaic:
      case render_parameters::lmmse_demosaic:
        if (!this->template rcd_interpolation_4x4<base_geometry::red,
                                                  base_geometry::green,
                                                  base_geometry::blue> (
                progress))
          return false;
