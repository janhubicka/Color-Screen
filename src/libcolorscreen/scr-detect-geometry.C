...
/* Detect a regular screen in IMG using color classification DPARAM and limits
   ...
   patches, and fit the final homography or mesh.  CAPTURE_RPARAM optionally
   supplies native-channel capture sharpening; other render adjustments are
   ignored.  Update SPARAM with final solver points, report optional diagnostics
   to REPORT_FILE, and use PROGRESS for status and cancellation.  Return an
   unsuccessful result on any rejected or cancelled stage.  */
detected_screen
detect_regular_screen_1(const image_data &img, scr_detect_parameters &dparam,
  ...
  render_parameters detect_rparam;
  ...
  detect_rparam.gamma = dsparams->gamma;
  if (capture_rparam)
    detect_rparam.sharpen = capture_rparam->sharpen;
      ...
      if (!progress || !progress->cancel_requested()) {
        ...
        if (!render) {
          ...
          std::unique_ptr<render_scr_detect> new_render(
              new render_scr_detect(dparam, img, detect_rparam, 256));
          if (!new_render) {
...
/* Detect a regular screen without capture sharpening and prune temporary
   render-detection caches before returning.  IMG, DPARAM, SPARAM, DSPARAMS,
   PROGRESS, and REPORT_FILE have the same roles as in detect_regular_screen_1().
   This wrapper preserves the historical public entry point.  */
detected_screen
...
/* Detect a regular screen, optionally applying CAPTURE_RPARAM's native-channel
   capture sharpening before screen-color classification.  IMG, DPARAM, SPARAM,
   DSPARAMS, PROGRESS, and REPORT_FILE have the same roles as in
   detect_regular_screen_1().  */
detected_screen
detect_regular_screen(const image_data &img, scr_detect_parameters &dparam,
                      solver_parameters &sparam,
                      const detect_regular_screen_params *dsparams,
