...
bool
analyze_base_worker<GEOMETRY>::analyze_precise (scr_to_img *scr_to_img, render_to_scr *render, const screen *screen, luminosity_t collection_threshold, luminosity_t *w_red, luminosity_t *w_green, luminosity_t *w_blue, int minx, int miny, int maxx, int maxy, progress_info *progress)
{
	  ...
	  for (int x = minx; x < maxx; x++)
	    {
		 ...
		 add extra bffer of 1 screen tile to be sure we do not access uninitialized memory.  */
	      if (!GEOMETRY::check_range && (scr.x <= 0 || scr.x >= m_width - 1 || scr.y <= 0 || scr.y >= m_height - 1))
		continue;
...
#pragma omp for nowait
      for (int y = 0; y < m_height; y++)
	{
	  if (!progress || !progress->cancel_requested ())
		...
		{
		  data_entry e = {x, y * GEOMETRY::green_height_scale + yy};
		  int idx = e.y * m_width * GEOMETRY::green_width_scale + e.x;
		  if (w_green [idx] != 0)
...
template<typename GEOMETRY>
bool
analyze_base_worker<GEOMETRY>::analyze_precise_rgb (scr_to_img *scr_to_img, render_to_scr *render, const screen *screen, luminosity_t collection_threshold, luminosity_t *w_red, luminosity_t *w_green, luminosity_t *w_blue, int minx, int miny, int maxx, int maxy, progress_info *progress)
{
#pragma omp parallel shared(progress, render, scr_to_img, screen, collection_threshold, w_blue, w_red, w_green, minx, miny, maxx, maxy) default(none)
  {
#pragma omp for 
...
template<typename GEOMETRY>
bool
analyze_base_worker<GEOMETRY>::analyze_color (scr_to_img *scr_to_img, render_to_scr *render, luminosity_t *w_red, luminosity_t *w_green, luminosity_t *w_blue, int minx, int miny, int maxx, int maxy, progress_info *progress)
{
	...
	if (!progress || !progress->cancel_requested ())
	  for (int x = minx; x < maxx; x++)
		...
		continue;
	      ...
	      point_t off;
	      analyze_base::data_entry e = GEOMETRY::red_scr_to_entry (scr, &off);
		  ...
		  && e.y + red_miny >= 0 && e.y + red_maxy < m_height * GEOMETRY::red_height_scale)
		{
		  ...
		  luminosity_t &l1 = w_red [(e.y) * m_width * GEOMETRY::red_width_scale + e.x];
		  luminosity_t &v1 = m_red [(e.y) * m_width * GEOMETRY::red_width_scale + e.x];
		  luminosity_t val1 = (1 - off.x) * (1 - off.y);
#pragma omp atomic
		  l1 += val1;
#pragma omp atomic
		  v1 += d.red * val1;
		  data_entry o = GEOMETRY::offset_for_interpolation_red (e, {1, 0});
		  luminosity_t &l2 = w_red [o.y * m_width * GEOMETRY::red_width_scale + o.x];
		  luminosity_t &v2 = m_red [o.y * m_width * GEOMETRY::red_width_scale + o.x];
...
#pragma omp atomic
		  ...
		  luminosity_t &l3 = w_red [o.y * m_width * GEOMETRY::red_width_scale + o.x];
		  luminosity_t &v3 = m_red [o.y * m_width * GEOMETRY::red_width_scale + o.x];
...
#pragma omp atomic
		  ...
		  luminosity_t &l4 = w_red [o.y * m_width * GEOMETRY::red_width_scale + o.x];
		  luminosity_t &v4 = m_red [o.y * m_width * GEOMETRY::red_width_scale + o.x];
...
#pragma omp atomic
		  ...
		  && e.y + green_miny >= 0 && e.y + green_maxy < m_height * GEOMETRY::green_height_scale)
		{
		  off.x = half_weights[(int)(off.x * 255.5)];
		  off.y = weights[(int)(off.y * 255.5)];
		  luminosity_t &l1 = w_green [(e.y) * m_width * GEOMETRY::green_width_scale + e.x];
		  luminosity_t &v1 = m_green [(e.y) * m_width * GEOMETRY::green_width_scale + e.x];
		  luminosity_t val1 = (1 - off.x) * (1 - off.y);
#pragma omp atomic
		  l1 += val1;
#pragma omp atomic
		  v1 += d.green * val1;
		  data_entry o = GEOMETRY::offset_for_interpolation_green (e, {1, 0});
		  luminosity_t &l2 = w_green [o.y * m_width * GEOMETRY::green_width_scale + o.x];
		  luminosity_t &v2 = m_green [o.y * m_width * GEOMETRY::green_width_scale + o.x];
		  luminosity_t val2 = (off.x) * (1 - off.y);
#pragma omp atomic
		  l2 += val2;
#pragma omp atomic
		  ...
		  luminosity_t &l3 = w_green [o.y * m_width * GEOMETRY::green_width_scale + o.x];
		  luminosity_t &v3 = m_green [o.y * m_width * GEOMETRY::green_width_scale + o.x];
		  luminosity_t val3 = (1 - off.x) * (off.y);
#pragma omp atomic
		  l3 += val3;
#pragma omp atomic
		  ...
		  luminosity_t &l4 = w_green [o.y * m_width * GEOMETRY::green_width_scale + o.x];
		  luminosity_t &v4 = m_green [o.y * m_width * GEOMETRY::green_width_scale + o.x];
		  luminosity_t val4 = (off.x) * (off.y);
#pragma omp atomic
		  l4 += val4;
#pragma omp atomic
		  ...
		  && e.y + blue_miny >= 0 && e.y + blue_maxy < m_height * GEOMETRY::blue_height_scale)
		{
		  off.x = half_weights[(int)(off.x * 255.5)];
		  off.y = weights[(int)(off.y * 255.5)];
		  luminosity_t &l1 = w_blue [(e.y) * m_width * GEOMETRY::blue_width_scale + e.x];
		  luminosity_t &v1 = m_blue [(e.y) * m_width * GEOMETRY::blue_width_scale + e.x];
		  luminosity_t val1 = (1 - off.x) * (1 - off.y);
#pragma omp atomic
		  l1 += val1;
#pragma omp atomic
		  v1 += d.blue * val1;
		  data_entry o = GEOMETRY::offset_for_interpolation_blue (e, {1, 0});
		  luminosity_t &l2 = w_blue [o.y * m_width * GEOMETRY::blue_width_scale + o.x];
		  luminosity_t &v2 = m_blue [o.y * m_width * GEOMETRY::blue_width_scale + o.x];
		  luminosity_t val2 = (off.x) * (1 - off.y);
#pragma omp atomic
		  l2 += val2;
#pragma omp atomic
		  ...
		  luminosity_t &l3 = w_blue [o.y * m_width * GEOMETRY::blue_width_scale + o.x];
		  luminosity_t &v3 = m_blue [o.y * m_width * GEOMETRY::blue_width_scale + o.x];
		  luminosity_t val3 = (1 - off.x) * (off.y);
#pragma omp atomic
		  l3 += val3;
#pragma omp atomic
		  ...
		  luminosity_t &l4 = w_blue [o.y * m_width * GEOMETRY::blue_width_scale + o.x];
		  luminosity_t &v4 = m_blue [o.y * m_width * GEOMETRY::blue_width_scale + o.x];
		  luminosity_t val4 = (off.x) * (off.y);
#pragma omp atomic
		  l4 += val4;
#pragma omp atomic
...
template<typename GEOMETRY>
bool
analyze_base_worker<GEOMETRY>::analyze_fast (render_to_scr *render,progress_info *progress)
{
	...
	for (int y = 0 ; y < m_height; y++)
	  {
		...
		{
		  data_entry e = {x * GEOMETRY::red_width_scale + xx, y * GEOMETRY::red_height_scale + yy};
		  int idx = e.y * m_width * GEOMETRY::red_width_scale + e.x;
		  point_t scr = GEOMETRY::red_entry_to_scr (e);
		  m_red [idx] = render->get_unadjusted_img_pixel_scr (scr.x - m_xshift, scr.y - m_yshift);
		}
		...
		{
		  data_entry e = {x * GEOMETRY::green_width_scale + xx, y * GEOMETRY::green_height_scale + yy};
		  int idx = e.y * m_width * GEOMETRY::green_width_scale + e.x;
		  point_t scr = GEOMETRY::green_entry_to_scr (e);
		  m_green [idx] = render->get_unadjusted_img_pixel_scr (scr.x - m_xshift, scr.y - m_yshift);
		}
		...
		{
		  data_entry e = {x * GEOMETRY::blue_width_scale + xx, y * GEOMETRY::blue_height_scale + yy};
		  int idx = e.y * m_width * GEOMETRY::blue_width_scale + e.x;
		  point_t scr = GEOMETRY::blue_entry_to_scr (e);
		  m_blue [idx] = render->get_unadjusted_img_pixel_scr (scr.x - m_xshift, scr.y - m_yshift);
		}
...
template<typename GEOMETRY>
bool
analyze_base_worker<GEOMETRY>::analyze (render_to_scr *render, const image_data *img, scr_to_img *scr_to_img, const screen *screen, int width, int height, int xshift, int yshift, mode mode, luminosity_t collection_threshold, progress_info *progress)
{
  assert (!m_red);
  m_width = width;
  m_height = height;
  ...
  if (mode != precise_rgb)
    {
      m_red = (luminosity_t *)calloc (m_width * m_height * (GEOMETRY::red_width_scale * GEOMETRY::red_height_scale),  sizeof (luminosity_t));
      m_green = (luminosity_t *)calloc (m_width * m_height * (GEOMETRY::green_width_scale * GEOMETRY::green_height_scale), sizeof (luminosity_t));
      m_blue = (luminosity_t *)calloc (m_width * m_height * (GEOMETRY::blue_width_scale * GEOMETRY::blue_height_scale), sizeof (luminosity_t));
      if (!m_red || !m_green || !m_blue)
	return false;
    }
  else
    {
      m_rgb_red = (rgbdata *)calloc (m_width * m_height * (GEOMETRY::red_width_scale * GEOMETRY::red_height_scale),  sizeof (rgbdata));
      m_rgb_green = (rgbdata *)calloc (m_width * m_height * (GEOMETRY::green_width_scale * GEOMETRY::green_height_scale), sizeof (rgbdata));
      m_rgb_blue = (rgbdata *)calloc (m_width * m_height * (GEOMETRY::blue_width_scale * GEOMETRY::blue_height_scale), sizeof (rgbdata));
  ...
  if (mode == precise || mode == precise_rgb || mode == color)
    {
      luminosity_t *w_red = (luminosity_t *)calloc (m_width * m_height * (GEOMETRY::red_width_scale * GEOMETRY::red_height_scale), sizeof (luminosity_t));
      luminosity_t *w_green = (luminosity_t *)calloc (m_width * m_height * (GEOMETRY::green_width_scale * GEOMETRY::green_height_scale), sizeof (luminosity_t));
      luminosity_t *w_blue = (luminosity_t *)calloc (m_width * m_height * (GEOMETRY::blue_width_scale * GEOMETRY::blue_height_scale), sizeof (luminosity_t));
	...
	if (mode == precise)
      ...
      if (mode == precise)
        ...
        ok = analyze_precise_rgb (scr_to_img, render, screen, collection_threshold, w_red, w_green, w_blue, minx, miny, maxx, maxy, progress);
      else
        ok = analyze_color (scr_to_img, render, w_red, w_green, w_blue, minx, miny, maxx, maxy, progress);
    ...
    {
      if (progress)
	progress->set_task ("determining intensities of color screen patches (fast mode)", m_height);
