...
public:
  ...
  /* Return number of radial shape coefficients fitted by the selected model.
     The edge-scale gauge is removed by normalize(), so Standard intentionally
     has only one free radial shape degree of freedom. */
  int
  num_radial_values () const
  {
  ...
  num_values () const
  {
    return num_coordinates () + num_radial_values ();
  }
  ...
  constrain (coord_t *vals) const
  {
    ...
    for (int i = n, j = 0; i < n + num_radial_values (); i++, j++)
      {
                    ...
                    lens_warp_correction_parameters *lens) const
  {
    ...
    *lens = lens_warp_correction_parameters ();
    ...
    lens->kr[2] = num_radial_values () > 1
                      ? vals[n + 1] * (1 / scale_kr)
                      : (coord_t)0;
    lens->kr[3] = num_radial_values () > 2
                      ? vals[n + 2] * (1 / scale_kr)
                      : (coord_t)0;
    if (!lens->is_monotone ())
      {
  ...
  const bool optimize_lens
      = sparam.optimize_lens
        && sparam.lens_optimization_sufficient (
               ...
               param->scanner_type, ignore_lens_coverage);
