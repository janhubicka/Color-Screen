...
bool
analyze_base::find_best_match_using_cpfind (
    analyze_base &other, coord_t *xshift_ret, coord_t *yshift_ret,
    int direction, scr_to_img &map, scr_to_img &other_map, int scale,
    FILE *report_file, progress_info *progress)
{
  ...
  if (!write_screen (screen1, NULL, &error, progress, rmin, rmax, gmin, gmax,
                     bmin, bmax)
      || !other.write_screen (screen2, NULL, &error, progress, rmin, rmax,
                              gmin, gmax, bmin, bmax))
    {
      if (progress)
  ...
  fprintf (
      ...
      "i w%i h%i f0 v28.53 Ra0 Rb0 Rc0 Rd0 Re0 Eev0 Er1 Eb1 r0 p0 y0 TrX0 "
      "TrY0 TrZ0 Tpy0 Tpp0 j0 a0 b0 c0 d0 e0 g0 t0 Va1 Vb0 Vc0 Vd0 Vx0 Vy0  "
      "Vm5 n\"%s\"\n"
      "i w%i h%i f0 v=0 Ra=0 Rb=0 Rc=0 Rd=0 Re=0 Eev0 Er1 Eb1 r0 p0 y0 "
      "TrX0.334802534835735 TrY0.00196425708222156 TrZ0 Tpy-0 Tpp0 j0 a=0 b=0 "
  ...
  for (int m = 0; m < 2; m++)
    {
      ...
      // project-cpfind-hor-out.pto >cpfind.out"))
      if (system (cmd))
        {
          if (progress)
            progress->pause_stdout ();
      ...
      f = fopen (direction ? "project-cpfind-vert-out.pto"
                           : "project-cpfind-hor-out.pto",
                 "r");
      if (!f)
        {
          if (progress)
      ...
      while (!feof (f))
        {
          int c;
          if ((c = fgetc (f)) != 'c' || (c = fgetc (f)) != ' '
              || (c = fgetc (f)) != 'n' || (c = fgetc (f)) != '0'
              || (c = fgetc (f)) != ' ' || (c = fgetc (f)) != 'N'
              || (c = fgetc (f)) != '1' || (c = fgetc (f)) != ' '
              || (c = fgetc (f)) != 'x')
            {
              while (c != EOF && c != '\n')
                c = fgetc (f);
	  ...
	  if (fscanf (f, "x%f y%f X%f Y%f to\n", &x1, &y1, &x2, &y2) != 4)
	    {
          ...
          if (direction >= 0)
            {
              ...
              if ((direction == 0
                   && (imgp.x < 0 || fabs (imgp.y) > fabs (imgp.x) / 5))
                  || (direction == 1
                      && (imgp.y < 0 || fabs (imgp.x) > fabs (imgp.y) / 5)))
                {
                  fprintf (
                      report_file,
                      "Control point %f %f %f %f identified by cpfind "
                      "discarded since offset is in wrong direction %f %f\n",
          ...
          if (my_fabs ((x2 - x1) - (coord_t) xo) > (coord_t) 0.3 || my_fabs ((y2 - y1) - (coord_t) yo) > (coord_t) 0.3)
            {
              if (report_file)
                fprintf (report_file,
                         "Control point %f %f %f %f identified by cpfind "
                         "discarded since offset is not integer\n",
                         x1, y1, x2, y2);
              bad_off.push_back ({ x2 - x1, y2 - y1 });
              continue;
            }
          /* For Paget screens only shifts with even sum makes sense
             TODO: If we ever support other scaled screens than pagets, this
          ...
          if (scale == 2 && ((xo + yo) & 1))
            {
              if (report_file)
                fprintf (report_file,
                         "Control point %f %f %f %f identified by cpfind "
                         "discarded since sum is not even\n",
                         x1, y1, x2, y2);
              bad_off.push_back ({ x2 - x1, y2 - y1 });
              continue;
            }
          if (report_file)
            fprintf (report_file,
                     "Control point: x1 %f y2 %f x2 %f y2 %f offsetx %i "
                     "offsety %i\n",
                     x1, y1, x2, y2, xo, yo);
          bool found = false;
          for (offsets &o : off)
      ...
      if (off.size ())
        {
          ...
          if (real_n > std::min (npoints / 3, 2))
            ...
            fprintf (report_file,
                     "cpfind result does not seem reliable. Best match is "
                     "only %i points out of %i\n",
                     max.n, npoints);
        }
    }
  if (report_file)
    fprintf (
        report_file,
        "cpfind failed to find useful points; trying to find match myself\n");
  return false;
}

int
analyze_base::find_best_match (int percentage, int max_percentage,
                               analyze_base &other, int cpfind,
                               coord_t *xshift_ret, coord_t *yshift_ret,
                               int direction, scr_to_img &map,
                               scr_to_img &other_map, FILE *report_file,
                               progress_info *progress)
{
  ...
  if (cpfind)
    {
      if (find_best_match_using_cpfind (other, xshift_ret, yshift_ret,
                                        direction, map, other_map, 1,
...
#pragma omp parallel for default(none)                                        \
    shared(progress, xstart, xend, ystart, yend, other, percentage,           \
               max_percentage, found, best_sqsum, best_xshift, best_yshift,   \
          ...
          if (direction >= 0)
            {
              ...
              if ((direction == 0
                      ...
                      && (imgp.y < 0 || fabs (imgp.x) > fabs (imgp.y) / 5)))
                {
                  if (is_cpfind)
                    fprintf (report_file,
                             "cpfind offset in wrong direction %f %f\n",
                             imgp.x, imgp.y);
                  continue;
                }
            }
		  ...
		  &&  (fabs (xi) > fabs (yi)/8))
		{
          ...
          if ((xxend - xxstart) * (yyend - yystart) * 100
                    ...
                    * percentage)
            {
              if (is_cpfind)
                fprintf (report_file,
                         "cpfind overlap too small test 1 max:%i known:%i "
                         "(ranges %i...%i %i...%i)\n",
                         (xxend - xxstart) * (yyend - yystart),
                         std::min (m_n_known_pixels, other.m_n_known_pixels),
          ...
          int ystep = 1;
#endif
          luminosity_t rsum1 = 0, rsum2 = 0, gsum1 = 0, gsum2 = 0, bsum1 = 0,
                       bsum2 = 0;
          ...
          if (est_noverlap * 100
                    ...
                    * percentage)
            {
              if (is_cpfind)
                fprintf (report_file,
                         "cpfind overlap too small test 2 estimated "
                         "overlap:%i known %i\n",
                         est_noverlap,
                         std::min (m_n_known_pixels, other.m_n_known_pixels));
              continue;
          ...
          if (est_noverlap * 100
              > std::max (m_n_known_pixels, other.m_n_known_pixels)
                    * max_percentage)
            {
              if (is_cpfind)
                fprintf (report_file,
                         "cpfind overlap too large test 2 estimated "
                         "overlap:%i known %i\n",
                         est_noverlap,
                         std::min (m_n_known_pixels, other.m_n_known_pixels));
              continue;
		...
		printf ("cpfind overlap too small test 3 overlap:%i steps %i %i known %i\n", noverlap, xstep, ystep, std::min (m_n_known_pixels, other.m_n_known_pixels));
          ...
          if (my_fabs (rscale - (luminosity_t) 1) > exposure_tolerance
              ...
              || my_fabs (bscale - (luminosity_t) 1) > exposure_tolerance)
            {
              if (is_cpfind)
                fprintf (report_file,
                         "cpfind answer rejected because of overall density "
                         "(red %f:%f %f green %f:%f %f blue %f:%f %f\n",
                         rsum1, rsum2, rscale, gsum1, gsum2, gscale, bsum1,
                         bsum2, bscale);
              continue;
            }
          if (is_cpfind)
...
#endif
                  luminosity_t rdiff1
                      = red_avg (x1, y1) * rscale - other.red_avg (x2, y2);
                  luminosity_t gdiff
                      = green_avg (x1, y1) * gscale - other.green_avg (x2, y2);
                  luminosity_t bdiff
                      = blue_avg (x1, y1) * bscale - other.blue_avg (x2, y2);
                  // sqsum += fabs (rdiff1) + fabs (rdiff2) + fabs (gdiff) +
                  // fabs (bdiff);
                  rdiff1 *= 65546;
                  gdiff *= 65536;
                  bdiff *= 65536;
...
#pragma omp critical
  ...
  if (found && report_file)
    fprintf (report_file,
             "Best match on offset %i,%i (final %f, %f0 sqsum %f scales "
             "%f,%f,%f overlap %f%%\n",
             best_xshift, best_yshift, finalp.x, finalp.y, best_sqsum,
             best_rscale, best_gscale, best_bscale,
             100.0 * best_noverlap
                 / std::min (m_n_known_pixels, other.m_n_known_pixels));
  if (val_known && (*xshift_ret != best_xshift || *yshift_ret != best_yshift))
    {
      if (progress)
        progress->pause_stdout ();
      point_t finalp1
          = map.scr_to_final ({ (coord_t)*xshift_ret, (coord_t)*yshift_ret });
      if (report_file)
        fprintf (report_file,
                 "Mismatch with cpfind: %f,%f (final: %f,%f) compared to "
                 "%i,%i (final: %f,%f)\n",
                 *xshift_ret, *yshift_ret, finalp1.x, finalp1.y, best_xshift,
                 best_yshift, finalp.x, finalp.y);
      printf ("Mismatch with cpfind: %f,%f (final: %f,%f) compared to %i,%i "
              "(final: %f,%f)\n",
              *xshift_ret, *yshift_ret, finalp1.x, finalp1.y, best_xshift,
              best_yshift, finalp.x, finalp.y);
      if (progress)
          ...
          for (int xx = xxstart; xx < xxend; xx++)
            {
              ...
              // fprintf (redf, "%f %f\n", red (x1 * 2, y1), other.red (x2 * 2,
              // y2)); fprintf (redf, "%f %f\n", red (x1 * 2+ 1, y1), other.red
              // (x2 * 2+ 1, y2)); fprintf (greenf, "%f %f\n", green (x1, y1),
              // other.green (x2, y2)); fprintf (bluef, "%f %f\n", blue (x1,
              // y1), other.blue (x2, y2));
            }
        }
      const char *error;
...
bool
analyze_base::write_screen (const char *filename, bitmap_2d *known_pixels,
                            const char **error, progress_info *progress,
                            luminosity_t rmin, luminosity_t rmax,
                            luminosity_t gmin, luminosity_t gmax,
                            luminosity_t bmin, luminosity_t bmax)
{
  tiff_writer_params p;
  p.filename = filename;
          ...
          if ((known_pixels ? known_pixels : m_known_pixels.get ())->test_bit (x, y))
            {
              out.row16bit ()[x * 4 + 0] = std::max (
                  std::min (linear_to_srgb ((red_avg (x, y) - rmin) * rscale)
                            ...
                            65535.0),
                  0.0);
              out.row16bit ()[x * 4 + 1] = std::max (
                  std::min (linear_to_srgb ((green_avg (x, y) - gmin) * gscale)
                                * 65535.5,
                            65535.0),
                  0.0);
              out.row16bit ()[x * 4 + 2] = std::max (
                  std::min (linear_to_srgb ((blue_avg (x, y) - bmin) * bscale)
                                * 65535.5,
                            65535.0),
                  0.0);
...
void
analyze_base::analyze_range (luminosity_t *rrmin, luminosity_t *rrmax,
                             luminosity_t *rgmin, luminosity_t *rgmax,
                             luminosity_t *rbmin, luminosity_t *rbmax)
{
...
analyze_base::denoise_red (const denoise_parameters &params, progress_info *progress)
{
  ...
  if (m_red_support)
    return colorscreen::denoise_screen_with_support<luminosity_t> (
        w, h, [&] (int x, int y) { return m_red[y * w + x]; },
        [&] (int x, int y) { return m_red_support[y * w + x]; },
        [&] (int x, int y, luminosity_t val) { m_red[y * w + x] = val; },
        m_red_entry_to_scr, m_red_scr_to_entry, 1 << m_rwscl, 1 << m_rhscl,
        params, progress);
  return colorscreen::denoise_screen<luminosity_t> (
      w, h, [&] (int x, int y) { return m_red[y * w + x]; },
      [&] (int x, int y, luminosity_t val) { m_red[y * w + x] = val; },
      m_red_entry_to_scr, m_red_scr_to_entry, 1 << m_rwscl, 1 << m_rhscl,
      params, progress);
...
analyze_base::denoise_green (const denoise_parameters &params, progress_info *progress)
{
  ...
  if (m_green_support)
    return colorscreen::denoise_screen_with_support<luminosity_t> (
        ...
        m_green_entry_to_scr, m_green_scr_to_entry, 1 << m_gwscl, 1 << m_ghscl,
        params, progress);
  return colorscreen::denoise_screen<luminosity_t> (
      w, h, [&] (int x, int y) { return m_green[y * w + x]; },
      [&] (int x, int y, luminosity_t val) { m_green[y * w + x] = val; },
...
analyze_base::denoise_blue (const denoise_parameters &params, progress_info *progress)
{
  ...
  if (m_blue_support)
    return colorscreen::denoise_screen_with_support<luminosity_t> (
        ...
        m_blue_entry_to_scr, m_blue_scr_to_entry, 1 << m_bwscl, 1 << m_bhscl,
        params, progress);
  return colorscreen::denoise_screen<luminosity_t> (
      w, h, [&] (int x, int y) { return m_blue[y * w + x]; },
      [&] (int x, int y, luminosity_t val) { m_blue[y * w + x] = val; },
...
analyze_base::denoise_rgb_red (const denoise_parameters &params, progress_info *progress)
{
  ...
  int h = m_area.height << m_rhscl;
  if (m_red_support)
    return colorscreen::denoise_screen_rgb_with_support (
        w, h, [&] (int x, int y) { return m_rgb_red[y * w + x]; },
        [&] (int x, int y) { return m_red_support[y * w + x]; },
        [&] (int x, int y, rgbdata val) { m_rgb_red[y * w + x] = val; },
        m_red_entry_to_scr, m_red_scr_to_entry, 1 << m_rwscl, 1 << m_rhscl,
        params, progress);
  return colorscreen::denoise_screen_rgb (
      w, h, [&] (int x, int y) { return m_rgb_red[y * w + x]; },
      [&] (int x, int y, rgbdata val) { m_rgb_red[y * w + x] = val; },
      m_red_entry_to_scr, m_red_scr_to_entry, 1 << m_rwscl, 1 << m_rhscl,
      params, progress);
...
bool
analyze_base::estimate_noise_scale_models (
    denoise_noise_scale_estimate *red_est,
    denoise_noise_scale_estimate *green_est,
    denoise_noise_scale_estimate *blue_est) const
{
  auto scalar = [&] (const luminosity_t *data, const luminosity_t *support,
                     int wscl, int hscl, entry_to_scr_fn entry_to_scr)
  {
    if (!data)
      return denoise_noise_scale_estimate{};
    const int w = m_area.width << wscl;
    const int h = m_area.height << hscl;
    return estimate_screen_noise_scale_model (
        w, h, [&] (int x, int y) { return data[(size_t)y * w + x]; },
        [&] (int x, int y)
        {
          return support ? support[(size_t)y * w + x] : (luminosity_t)1;
                  ...
                  int wscl, int hscl, entry_to_scr_fn entry_to_scr)
  {
    ...
    const int w = m_area.width << wscl;
    const int h = m_area.height << hscl;
    return estimate_screen_rgb_noise_scale_model (
        w, h, [&] (int x, int y) { return data[(size_t)y * w + x]; },
        [&] (int x, int y)
        {
          return support ? support[(size_t)y * w + x] : (luminosity_t)1;
  ...
  denoise_noise_scale_estimate r, g, b;
  if (m_rgb_red || m_rgb_green || m_rgb_blue)
    {
      r = rgb (m_rgb_red.get (), m_red_support.get (), m_rwscl, m_rhscl,
               m_red_entry_to_scr);
...
/* Compare noise-estimator domains for the RED_EST, GREEN_EST and BLUE_EST
   collected separations without changing rendering state.  */
bool
