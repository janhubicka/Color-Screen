...
/* Cache helper for adjusted RGB data used by slow image-domain confirmation.
...
render_scr_detect::precompute_all (int flags, progress_info *progress)
{
  ...
  /* Capture sharpening is part of the normal render pipeline.  Prepare the
     sharpened native RGB channels before screen classification so autodetect
     sees the same capture correction as the rest of the application.  */
  ...
  color_class_params p = {m_precomputed_rgbdata ? m_precomputed_rgbdata_id
                                                : m_img.id,
                          &m_img, m_precomputed_rgbdata,
                          m_scr_detect.m_param, &m_scr_detect, m_params.gamma};
  if (progress && progress->cancel_requested ())
    return false;
...
render_scr_detect::precompute_rgbdata (progress_info *progress)
{
  ...
  struct precomputed_rgbdata_params p
      = { m_img.id, m_rgb_image ? m_rgb_image_id : m_img.id,
          m_scr_detect.m_param, m_params.gamma, &m_img, &m_scr_detect, this };
