...
stitch_image::analyze (stitch_project *prj, detect_regular_screen_params *dsparamsptr, bool top_p, bool bottom_p, bool left_p, bool right_p, lens_warp_correction_parameters &lens_correction, progress_info *progress)
{
  ...
  int img_layer_c = my_rparam.get_image_layer_channel(img.get());
  ...
  if (dufay_like_screen_p (param.type))
    {
      ...
      dufay->analyze (&render, img.get(), &scr_to_img_map, m_prj->my_screen.get(), NULL, {-xshift, -yshift, width, height}, analyze_base::precise, 0.7, progress);
  ...
  else
    {
      ...
      paget->analyze (&render, img.get(), &scr_to_img_map, m_prj->my_screen.get(), NULL, {-xshift, -yshift, width, height}, analyze_base::precise, 0.7, progress);
