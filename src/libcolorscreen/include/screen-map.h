...
{
public:
  DLL_PUBLIC screen_map (enum scr_type type1, int xshift1, int yshift1, int width1, int height1);
  DLL_PUBLIC ~screen_map ();
  DLL_PUBLIC bool write_outliers_info (const char *filename, int width, int scale, int height, scr_to_img &map, const char **error, progress_info *progress);
  ...
  void get_screen_coord (int x, int y, coord_t *scr_x, coord_t *scr_y, solver_parameters::point_color *color = NULL)
  {
	...
	*color = (x & 1) ? solver_parameters::blue : solver_parameters::green;
      ...
      {
        int_point_t p = paget_geometry::from_diagonal_coordinates (int_point_t{x, y});
	*scr_x = p.x / 4.0;
	*scr_y = p.y / 4.0;
	if (!color)
	  return;
  ...
  int check_consistency (FILE *out, coord_t coordinate1_x, coord_t coordinate1_y, coord_t coordinate2_x, coord_t coordinate2_y, coord_t tolerance);
  ...
  void determine_solver_points (int patches_found, solver_parameters *sparam);
  enum scr_type type;
