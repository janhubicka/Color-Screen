...
bool
sharpen(luminosity_t *out, T data, P param, int width, int height, luminosity_t radius, luminosity_t amount, progress_info *progress)
{
...
#pragma omp parallel shared(progress,out,clen,cmatrix,width, height, amount, param, data) default(none)
    {
      luminosity_t *hblur = (luminosity_t *)calloc (width * clen, sizeof (luminosity_t));
      ...
      for (int d = -clen/2; d < clen/2 - 1; d++)
	{
	  ...
	  int tp = (yp + clen) % clen;
	  if (yp < 0 || yp > height)
	    memset (hblur + tp * width, 0, sizeof (luminosity_t) * width);
	  else
	    fir_blur::blur_horisontal<T, P, getdata> (hblur + tp * width, data, param, yp, width, clen, cmatrix);
	}
      for (int y = ystart; y <= yend; y++)
	{
	  if (y + clen / 2 - 1 < height)
	    fir_blur::blur_horisontal<T, P, getdata> (hblur + ((y + clen / 2 - 1 + clen) % clen) * width, data, param, y + clen / 2 - 1, width, clen, cmatrix);
	  else
	    memset (hblur + ((y + clen / 2 - 1 + clen) % clen) * width, 0, sizeof (luminosity_t) * width);
	  ...
	  for (int x = 0; x < width; x++)
	    {
	      luminosity_t sum = 0;
	      for (int d = 0; d < clen; d++)
		sum += rotated_cmatrix[d] * hblur[d * width + x];
	      luminosity_t orig = getdata (data, x, y, width, param);
