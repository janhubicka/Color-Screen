...
static const char *bwcmd
    = "montage -geometry 300x300+10+10 /tmp/bworig.tif /tmp/bwsimulated.tif "
      "/tmp/bwdiff.tif /tmp/bwdot-spread.tif /tmp/bwscr.tif "
      "/tmp/bwscr-blur.tif "
...
#define SCALE 16
  ...
  struct _Data
  {
    ...
    GtkSpinButton *gamma, *scanner_mtf_sigma, *presaturation, *saturation,
        ...
        *RL_iterations, *sigma, *tilt_x, *tilt_y, *scanner_mtf_scale,
        *dark_point, *collection_threshold, *balance_black, *mix_red,
        *mix_green, *mix_blue, *balance_red, *balance_green, *balance_blue;
  };
  Data data;
  ...
  getvals (void)
  {
    ...
    rparams.sharpen.usm_radius
        = gtk_spin_button_get_value (data.sharpen_radius);
    rparams.sharpen.usm_amount
        = gtk_spin_button_get_value (data.sharpen_amount);
    // rparams.screen_blur_radius = gtk_spin_button_get_value
    // (data.screen_blur);
    ...
    rparams.white_balance.green
        = gtk_spin_button_get_value (data.balance_green);
    ...
    rparams.sharpen.richardson_lucy_sigma
        = gtk_spin_button_get_value (data.sigma);
  ...
  setvals (void)
  {
    ...
    gtk_spin_button_set_value (data.sharpen_radius,
                               rparams.sharpen.usm_radius);
    gtk_spin_button_set_value (data.sharpen_amount,
                               rparams.sharpen.usm_amount);
    ...
    gtk_spin_button_set_value (data.balance_green,
                               rparams.white_balance.green);
    ...
    gtk_spin_button_set_value (data.RL_iterations,
                               rparams.sharpen.richardson_lucy_iterations);
    gtk_spin_button_set_value (data.sigma,
                               rparams.sharpen.richardson_lucy_sigma);
    initialized = 1;
  }
  ...
  print_help ()
  {
    ...
    if (ui_mode == screen_editing)
      printf ("Screen editing mode\n"
              "c   - set center                              C   - set lens "
              "center\n"
              "x   - freeze x                                y   - freeze y   "
              "       "
              "            a - unfreeze both\n"
              "s S - fast/precise screen collection          O   - optimize "
              "colors   "
    ...
    if (ui_mode == screen_editing || ui_mode == solver_editing)
      ...
      printf (
          "d   - set dark point                         r g b- set color\n");
    printf (
        "W   - switch to solver editing mode           E   - screen editing "
        "mode                   \n"
  ...
  static void
  cb_image_annotate (GtkImageViewer *imgv, GdkPixbuf *pixbuf, gint shift_x,
                     gint shift_y, gdouble scale_x, gdouble scale_y,
                     gpointer user_data)
  {
    ...
    if (k == 'j')
      {
        if ((int)rparams.sharpen.mode
          ...
          rparams.sharpen.mode
              = (sharpen_parameters::sharpen_mode)((int)rparams.sharpen.mode
                                                   + 1);
        printf (
            "Sharpen algorithm: %s\n",
    ...
    if ((k == 'm' || k == 'M') && !(event->state & GDK_CONTROL_MASK))
      {
        if (k == 'm')
          {
            rparams.color_model
                = (render_parameters::color_model_t)((int)rparams.color_model
                                                     + 1);
            if ((int)rparams.color_model
        ...
        else
          {
            if ((int)rparams.color_model == 0)
              rparams.color_model
                  = (render_parameters::color_model_t)((int)render_parameters::
                                                           color_model_max
                                                       - 1);
            else
              rparams.color_model
        ...
        if (k == 'v')
          {
            rparams.output_tone_curve
                = (tone_curve::tone_curves)((int)rparams.output_tone_curve
                                            + 1);
        ...
        else
          {
            if ((int)rparams.output_tone_curve == 0)
              rparams.output_tone_curve
                  = (tone_curve::tone_curves)((int)tone_curve::tone_curve_max
                                              - 1);
            else
              rparams.output_tone_curve
                  = (tone_curve::tone_curves)((int)rparams.output_tone_curve
    ...
    if (k == ' ')
      {
        ...
        gtk_image_viewer_get_scale_and_shift (
            GTK_IMAGE_VIEWER (data.image_viewer), &scale_x, &scale_y, &shift_x,
            &shift_y);
        ...
        if (k == 'l' && !(event->state & GDK_CONTROL_MASK))
          {
            if (rparams.dye_balance == render_parameters::dye_balance_none)
              rparams.dye_balance
                  = (render_parameters::dye_balance_t)((int)render_parameters::
                                                           dye_balance_max
                                                       - 1);
            else
              rparams.dye_balance
                  = (render_parameters::dye_balance_t)((int)rparams.dye_balance
                                                       - 1);
            printf ("Dye balance:%s\n",
                    render_parameters::dye_balance_names[(int)rparams
                                                             .dye_balance]);
            display_scheduled = true;
            preview_display_scheduled = true;
          }
                  ...
                  if (!screen_with_vertical_strips_p (current.type))
                    {
                      ...
                      coord_t dist
                          = p.dist_from (current_solver.points[n].img);
                  ...
                  else
                    {
                      ...
                      coord_t dist
                          = fabs (p.x - current_solver.points[n].scr.x);
                      hist.pre_account (dist);
                    }
                }
        ...
        if (k == 'D' && scan.rgbdata)
          {
            ...
            detected
                = detect_regular_screen (scan, current_scr_detect,
                                         current_solver, &dsparams, &progress);
            if (detected.success)
              {
        ...
        if (k == 'l' && (event->state & GDK_CONTROL_MASK))
          {
            ...
            int ymax = std::max (sel1y, sel2y);
            bool ret;
            printf ("Auto levels in selection in selection %i %i %i %i old "
                    "darkpoint "
                    "%f old brightness %f\n",
                    xmin, ymin, xmax, ymax, rparams.dark_point,
                    rparams.brightness);
            {
              file_progress_info progress (stdout);
        ...
        if (k == 'f' && (event->state & GDK_MOD1_MASK))
          {
            ...
            fparam.flags |= finetune_position | finetune_bw | finetune_verbose
                            | finetune_emulsion_blur /*| finetune_dufay_strips
                                                        | finetune_fog*/
                ;
            file_progress_info progress (stdout);
            finetune_result res = finetune (rparams, current, scan,
                                            { { (coord_t)x, (coord_t)y } },
        ...
        if (k == 's' && (event->state & GDK_MOD1_MASK))
          {
            ...
            fparam.flags
                |= finetune_position
                   | finetune_verbose       /*| finetune_screen_mtf_blur*/
            ...
            file_progress_info progress (stdout);
            finetune_result res
                = finetune (rparams, current, scan, tune_points, &tune_results,
                            fparam, &progress);
            if (res.success)
              {
                rparams.screen_blur_radius = res.screen_blur_radius;
    ...
    if (ui_mode == screen_editing || ui_mode == solver_editing
        || ui_mode == color_profiling)
      {
        ...
        if (k == 'N')
          {
            ...
            file_progress_info progress (stdout);
            current_mesh
                = solver_mesh (&current, scan, current_solver, &progress);
            display_scheduled = true;
            preview_display_scheduled = true;
          }
        if (k == 'n')
        ...
        if ((event->state & GDK_CONTROL_MASK) && scan.stitch && k >= '1'
            && k <= '9')
          {
            ...
            if (rparams.tile_adjustments_width != scan.stitch->params.width
                || rparams.tile_adjustments_height
                       != scan.stitch->params.height)
              rparams.set_tile_adjustments_dimensions (
                  scan.stitch->params.width, scan.stitch->params.height);
        ...
        if (k == 't')
          {
            save_parameters ();
            current.scanner_type
                = (scanner_type)((int)current.scanner_type + 1);
            preview_display_scheduled = true;
            display_scheduled = true;
        ...
        if (k == 'X' && scan.has_rgb ())
          {
            rparams.compute_mix_weights (
                patch_proportions (current.type, &rparams));
            setvals ();
          }
      }
    else
      {
        if (k >= '1'
            && k <= (int)render_type_max - (int)render_type_first_scr_detect
                        + 1 + '1')
          {
        ...
        if (k == 'P' || k == 'O')
          {
            ...
            gint pysize = /*gtk_image_viewer_get_image_height (GTK_IMAGE_VIEWER
                ...
                800;
            gtk_image_viewer_get_scale_and_shift (
                GTK_IMAGE_VIEWER (data.image_viewer), &scale_x, &scale_y,
                &shift_x, &shift_y);
            int minx = std::max ((int)(shift_x / scale_x), 0);
            int maxx
                = std::min ((int)((shift_x + pxsize) / scale_x), scan.width);
            int miny = std::max ((int)(shift_y / scale_y), 0);
            int maxy
  ...
  std::string filename = path.parent_path() + "/../colorscreen/gtkgui.glade";
    ...
    if (!gtk_builder_add_from_file (builder,
      ...
      && !gtk_builder_add_from_file (builder, filename.c_str (), NULL))
        ...
        && !gtk_builder_add_from_file (
            builder, "../share/colorscreen/gtkgui.glade", NULL))
      {
        fprintf (stderr,
                 "Can not open " DATADIR "/colorscreen/gtkgui.glade\n");
        exit (1);
      }
    ...
    gtk_image_viewer_set_scroll_region (GTK_IMAGE_VIEWER (image_viewer), 20,
                                        20, scan.width - 20, scan.height - 20);
    gtk_image_viewer_set_zoom_range (GTK_IMAGE_VIEWER (image_viewer), 0.01,
                                     64);
    ...
    data.scanner_mtf_scale = GTK_SPIN_BUTTON (
        gtk_builder_get_object (builder, "scanner_mtf_scale"));
    data.dark_point
        = GTK_SPIN_BUTTON (gtk_builder_get_object (builder, "dark_point"));
    data.collection_threshold = GTK_SPIN_BUTTON (
        gtk_builder_get_object (builder, "collection_threshold"));
    data.balance_black
        = GTK_SPIN_BUTTON (gtk_builder_get_object (builder, "balance_black"));
    data.mix_red
        = GTK_SPIN_BUTTON (gtk_builder_get_object (builder, "mix_red"));
    data.mix_green
        = GTK_SPIN_BUTTON (gtk_builder_get_object (builder, "mix_green"));
    data.mix_blue
        = GTK_SPIN_BUTTON (gtk_builder_get_object (builder, "mix_blue"));
    data.balance_red
  ...
  previewrender (GdkPixbuf **pixbuf)
  {
    ...
    *pixbuf = gdk_pixbuf_new (GDK_COLORSPACE_RGB, 0, 8, my_xsize, my_ysize);
    ...
    file_progress_info progress (stdout);
    render_preview (scan, get_scr_to_img_parameters (), rparams, pixels,
                    my_xsize, my_ysize, rowstride, &progress);
  ...
  static void
  draw_circle (cairo_surface_t *surface, coord_t bigscale, int xoffset,
               int yoffset, int width, int height, coord_t x, coord_t y,
               coord_t r, coord_t g, coord_t b, coord_t radius = 3,
  ...
  static void
  draw_line (cairo_surface_t *surface, coord_t bigscale, int xoffset,
             int yoffset, coord_t x1, coord_t y1, coord_t x2, coord_t y2,
             coord_t r, coord_t g, coord_t b, coord_t width = 1,
             luminosity_t opacity = 0.5)
  {
    cairo_t *cr = cairo_create (surface);
    cairo_translate (cr, -xoffset, -yoffset);
  ...
  static void
  draw_rectangle (cairo_surface_t *surface, coord_t bigscale, int xoffset,
                  int yoffset, coord_t x1, coord_t y1, coord_t x2, coord_t y2,
                  coord_t r, coord_t g, coord_t b, coord_t width = 1,
                  luminosity_t opacity = 0.5)
  {
  ...
  void
  draw_text (cairo_surface_t *surface, coord_t bigscale, int xoffset,
  ...
  bigrender (int xoffset, int yoffset, coord_t bigscale, GdkPixbuf *bigpixbuf)
  {
...

    {
      ...
      ret = render_tile (scan, get_scr_to_img_parameters (),
                         current_scr_detect, rparams, rtparam, tile,
                         &progress);
    }
            ...
            if (detected.smap->known_p ({ xx, yy }))
              {
                ...
                draw_circle (surface, bigscale, xoffset, yoffset, pxsize,
                             pysize, img.x, img.y,
                             t == solver_parameters::blue,
        ...
        for (size_t i = 0; i < color_optimizer_points.size (); i++)
          {
            point_t p;
            if (!scan.stitch)
              p = map.to_img ({ color_optimizer_points[i].x,
                                color_optimizer_points[i].y });
            else
              {
                p = scan.stitch->common_scr_to_img.scr_to_final (
                    { color_optimizer_points[i].x,
                      color_optimizer_points[i].y });
                p.x -= scan.xmin;
                p.y -= scan.ymin;
              }
            ...
            if (color_optimizer_match.size () > i)
              {
                ...
                draw_text (surface, bigscale, xoffset, yoffset, buf,
                           p.x + 27 / bigscale, p.y + 2 / bigscale, 0, 0, 0,
                           1);
                draw_text (surface, bigscale, xoffset, yoffset, buf,
                           p.x + 25 / bigscale, p.y, 1, 1, 1, 1);
              }
            draw_circle (surface, bigscale, xoffset, yoffset, pxsize, pysize,
                         p.x, p.y, 1, 1, 1, 23 / bigscale, 1);
            draw_circle (surface, bigscale, xoffset, yoffset, pxsize, pysize,
                         p.x, p.y, c2.blue, c2.green, c2.red, 20 / bigscale,
                         1);
            draw_circle (surface, bigscale, xoffset, yoffset, pxsize, pysize,
                         p.x, p.y, c1.blue, c1.green, c1.red, 10 / bigscale,
                         1);
          }
      }
        ...
        for (int i = 0; i < current_solver.n_points (); i++)
          {
            ...
            /* If we do only 1D solving, provide correct y coordinate to
             * minimize distance.  */
            if (screen_with_vertical_strips_p (current.type))
              {
            ...
            draw_circle (surface, bigscale, xoffset, yoffset, pxsize, pysize,
                         xi, yi, color.blue, color.green, color.red);
            draw_circle (surface, bigscale, xoffset, yoffset, pxsize, pysize,
                         p.x, p.y, 3 * color.blue / 4, 3 * color.green / 4,
                         3 * color.red / 4);

            coord_t patch_diam
                = sqrt (current.coordinate1.x * current.coordinate1.x
            ...
            if (bad)
              draw_line (surface, bigscale, xoffset, yoffset, xi, yi, xi + xd,
                         yi + yd, 0, 0, 1, 16);
            draw_line (surface, bigscale, xoffset, yoffset, xi, yi, xi + xd,
                       yi + yd, color.blue, color.green, color.red, 6);
            draw_line (surface, bigscale, xoffset, yoffset, xi + xd, yi + yd,
                       xi + xd * 0.7 + yd * 0.2, yi + yd * 0.7 - xd * 0.2,
    ...
    if (ui_mode == solver_editing)
      {
        if (sel1x != sel2x || sel1y != sel2y)
          draw_rectangle (surface, bigscale, xoffset, yoffset, sel1x, sel1y,
                          sel2x, sel2y, 1, 1, 1, 6);
      }
    ...
    if (ui_mode == color_profiling)
      {
          ...
          ;
        {
          ...
          for (int i = 0; i < color_optimizer_points.size (); i++)
            {
              coord_t dist
                  = sqrt ((color_optimizer_points[i].x - screen.x)
        ...
        if (event->button == 3)
          {
            ...
            for (n = 0; n < current_solver.n_points (); n++)
              {
                double dist
                    = sqrt ((current_solver.points[n].img.x - x)
                                * (current_solver.points[n].img.x - x)
                            + (current_solver.points[n].img.y - y)
                                  * (current_solver.points[n].img.y - y));
                if (dist < 5 * std::min (scale_x, (gdouble)1)
                    && dist < best_dist)
                  {
                    best_n = n;
                    best_dist = dist;
  ...
  handle_drag (int x, int y, int button)
  {
    ...
    if (button == 1)
    ...
    else if (button == 3)
      {
        ...
        if (!freeze_x)
          {
            current.coordinate1.x
                = (press_parameters.coordinate1.x * cos (angle)
                   - press_parameters.coordinate1.y * sin (angle))
                  * scale;
            current.coordinate1.y
                = (press_parameters.coordinate1.x * sin (angle)
                   + press_parameters.coordinate1.y * cos (angle))
                  * scale;
          }
        if (!freeze_y)
          {
            current.coordinate2.x
                = (press_parameters.coordinate2.x * cos (angle)
                   - press_parameters.coordinate2.y * sin (angle))
                  * scale;
            current.coordinate2.y
                = (press_parameters.coordinate2.x * sin (angle)
        ...
        if (x == xpress && y == ypress)
          {
            ...
            if (scale_x < desired_zoom && event->button == 1)
              {
                ...
                display_type = 0;
                gtk_image_viewer_get_scale_and_shift (
                    GTK_IMAGE_VIEWER (data.image_viewer), &saved_scale_x,
                    &saved_scale_y, &saved_shift_x, &saved_shift_y);
                gtk_image_viewer_set_scale_and_shift (
                    GTK_IMAGE_VIEWER (data.image_viewer), desired_zoom,
                    desired_zoom, desired_zoom * x - event->x,
                    desired_zoom * y - event->y);
                return;
              }
            coord_t screenx, screeny;
            coord_t pscreenx, pscreeny;
            ...
            enum solver_parameters::point_color rcolor
            ...
            if (event->button == 1)
              {
                ...
                current_solver.add_point ({ x, y }, { rscreenx, rscreeny },
                                          rcolor);
#if 0
	      int n;
  ...
  main (int argc, char **argv)
  {
    ...
    const char *error;
    if (in
        && !load_csp (in, &current, &current_scr_detect, &rparams,
  ...
  current.mesh_trans->set_point (1,1, 100, 100);
    ...
    gtk_image_viewer_set_scale_and_shift (
        GTK_IMAGE_VIEWER ((GtkImageViewer *)data.image_viewer), scale, scale,
        0, 0);
    gtk_image_viewer_redraw ((GtkImageViewer *)data.image_viewer, 1);
