...
static const char *colorcmd =
    "montage -geometry 300x300+10+10 /tmp/colororig.tif "
    "/tmp/colorsimulated.tif /tmp/colordiff.tif /tmp/colordot-spread.tif "
    "/tmp/colorscr.tif /tmp/colorscr-blur.tif /tmp/colorscr-collected.tif  "
    "/tmp/color.tif ; display /tmp/color.tif &";
static const char *bwcmd =
    "montage -geometry 300x300+10+10 /tmp/bworig.tif /tmp/bwsimulated.tif "
    "/tmp/bwdiff.tif /tmp/bwdot-spread.tif /tmp/bwscr.tif /tmp/bwscr-blur.tif "
    "/tmp/bwscr-collected.tif /tmp/bw.tif ; display /tmp/bw.tif &";
...
static void bigrender(int xoffset, int yoffset, coord_t bigscale,
                      GdkPixbuf *bigpixbuf);
...
struct _Data {
  ...
  GtkSpinButton *gamma, *scanner_mtf_sigma, *presaturation, *saturation,
      *sharpen_radius, *sharpen_amount, *y2, *brightness, *scanner_snr,
      *RL_iterations, *sigma, *tilt_x, *tilt_y, *scanner_mtf_scale, *dark_point,
      *collection_threshold, *balance_black, *mix_red, *mix_green, *mix_blue,
...
G_MODULE_EXPORT void cb_press(GtkImage *image, GdkEventButton *event,
                              Data *data);
G_MODULE_EXPORT gboolean cb_delete_event(GtkWidget *window, GdkEvent *event,
                                         Data *data) {
  gint response = 1;
...
static void openimage(const char *name) {
  const char *error;
  bool ret;
  {
...
static void getvals(void) {
  ...
  rparams.sharpen.scanner_mtf.sigma =
      gtk_spin_button_get_value(data.scanner_mtf_sigma);
  ...
  rparams.sharpen.scanner_mtf_scale =
      gtk_spin_button_get_value(data.scanner_mtf_scale);
  rparams.dark_point = gtk_spin_button_get_value(data.dark_point);
  rparams.collection_threshold =
      gtk_spin_button_get_value(data.collection_threshold);
  rparams.backlight_correction_black =
      gtk_spin_button_get_value(data.balance_black);
  ...
  rparams.sharpen.richardson_lucy_iterations =
      gtk_spin_button_get_value(data.RL_iterations);
...
static void setvals(void) {
  ...
  gtk_spin_button_set_value(data.scanner_mtf_scale,
                            rparams.sharpen.scanner_mtf_scale);
  gtk_spin_button_set_value(data.dark_point, rparams.dark_point);
  gtk_spin_button_set_value(data.collection_threshold,
                            rparams.collection_threshold);
  gtk_spin_button_set_value(data.balance_black,
                            rparams.backlight_correction_black);
  ...
  gtk_spin_button_set_value(data.RL_iterations,
                            rparams.sharpen.richardson_lucy_iterations);
...
static void print_help() {
  ...
  if (ui_mode == color_profiling)
    printf("Color profiling mode\n"
           ...
           "p   - switch to screen editing                X   - set mixing "
           "weights\n");
  if (ui_mode == screen_detection)
    printf("Screen detection mode\n"
           "e   - switch to screen editing                P   - determine "
           "screen proportions\n"
           "                                              O   - determine "
           "screen proportions and assume screen geometry is correct\n"
           "d   - set black                              r g b- set given "
           "color\n");
  if (ui_mode == screen_editing)
    printf(
        "Screen editing mode\n"
        "c   - set center                              C   - set lens center\n"
        "x   - freeze x                                y   - freeze y          "
        "            a - unfreeze both\n"
        "s S - fast/precise screen collection          O   - optimize colors   "
        "            P - color profiling mode\n");
  if (ui_mode == solver_editing)
    printf("Solver editing mode\n"
           "w   - switch to screen editing mode\n"
           "D   - detect regular screen                   a A - autosolve      "
           "               L - set lens center\n"
           "      (finetune mixing in selection)\n"
           "l   - disable lens center			  dek - remove points "
           "in selected region\n"
           "ctrl+A - autodetect points in selection       ctrl+L - autodetect "
           "brightness in selection\n"
           "f   - finetune screen blur in selection using BW channel\n"
           "ctrl+f - finetune screen blur in selection using BW channel; with "
           "shift using color channel\n"
           "ctrl+s - finetune strips, screen blur, emulsion blur and fog; "
           "record sampling point\n"
           "alt+s -  finetune strips, emulsion blue, screen blue, fog and do "
           "not normaliz using points prevoiusly recorded in ctrl+s\n"
           "ctrl+w - set white balance so selection is neutral gray\n"
           "ctrl+d - set mix dark so color in selection\n"
           "ctrl+m - optimize mixing weights to balance selection to neutral "
           "gray\n"
  ...
  if (ui_mode == screen_editing || ui_mode == solver_editing)
    printf("                                              d   - set to dufay   "
           "               p - set to Paget\n"
           "f   - set to Finlay                           N   - compute mesh "
           "(nonlinear)      n - disable mesh\n"
           "1-9 - display modes                           t   - scanner type   "
           "               \n"
           "l L - dye balance\n");
  else
    printf("d   - set dark point                         r g b- set color\n");
  printf("W   - switch to solver editing mode           E   - screen editing "
         "mode                   \n"
         "o   - (simulated) infrared/color switch                              "
         "           u U - undo / redo\n"
         "m M - color models                            b B - light "
         "temperature      ctrl b B - backlight temperature\n"
...
static void cb_image_annotate(GtkImageViewer *imgv, GdkPixbuf *pixbuf,
                              gint shift_x, gint shift_y, gdouble scale_x,
                              gdouble scale_y, gpointer user_data) {
...
static gint cb_key_press_event(GtkWidget *widget, GdkEventKey *event) {
  ...
  if (k == 'j') {
    if ((int)rparams.sharpen.mode >=
        (int)sharpen_parameters::richardson_lucy_deconvolution - 1)
      rparams.sharpen.mode = sharpen_parameters::none;
    else
      rparams.sharpen.mode =
          (sharpen_parameters::sharpen_mode)((int)rparams.sharpen.mode + 1);
  ...
  if ((k == 'm' || k == 'M') && !(event->state & GDK_CONTROL_MASK)) {
    if (k == 'm') {
      rparams.color_model =
          (render_parameters::color_model_t)((int)rparams.color_model + 1);
      ...
      if ((int)rparams.color_model == 0)
        rparams.color_model =
            (render_parameters::
                 color_model_t)((int)render_parameters::color_model_max - 1);
      else
        rparams.color_model =
            (render_parameters::color_model_t)((int)rparams.color_model - 1);
    }
    printf("Color model: %s\n",
           render_parameters::color_model_properties[(int)rparams.color_model]
               .name);
    display_scheduled = true;
    preview_display_scheduled = true;
  }
  if (k == 'v' || k == 'V') {
    if (k == 'v') {
      rparams.output_tone_curve =
          (tone_curve::tone_curves)((int)rparams.output_tone_curve + 1);
      ...
      if ((int)rparams.output_tone_curve == 0)
        rparams.output_tone_curve =
            (tone_curve::tone_curves)((int)tone_curve::tone_curve_max - 1);
      else
        rparams.output_tone_curve =
            (tone_curve::tone_curves)((int)rparams.output_tone_curve - 1);
    }
  ...
  if (k == ' ') {
    ...
    gint pxsize = /*gtk_image_viewer_get_image_width (GTK_IMAGE_VIEWER
                     (data.image_viewer))*/
        1024;
    gint pysize = /*gtk_image_viewer_get_image_height (GTK_IMAGE_VIEWER
                     (data.image_viewer))*/
        800;
  ...
  if (k == 'b' && (event->state & GDK_CONTROL_MASK)) {
    rparams.backlight_temperature =
        std::max(rparams.backlight_temperature - 100,
                 (luminosity_t)render_parameters::temperature_min);
    printf("backlight temperature %f\n", rparams.backlight_temperature);
    display_scheduled = true;
    preview_display_scheduled = true;
  } else if (k == 'b') {
    rparams.temperature =
        std::max(rparams.temperature - 100,
                 (luminosity_t)render_parameters::temperature_min);
  ...
  if (k == 'B' && (event->state & GDK_CONTROL_MASK)) {
    rparams.backlight_temperature =
        std::min(rparams.backlight_temperature + 100,
                 (luminosity_t)render_parameters::temperature_max);
    printf("backlight temperature %f\n", rparams.backlight_temperature);
    display_scheduled = true;
    preview_display_scheduled = true;
  } else if (k == 'B') {
  ...
  if (k == 'G' && scan.stitch && (event->state & GDK_CONTROL_MASK)) {
    ...
    file_progress_info progress(stdout);
    const char *error;
    if (!scan.stitch->optimize_tile_adjustments(
            &rparams,
            stitch_project::
                    OPTIMIZE_ALL /*& (~stitch_project::OPTIMIZE_EXPOSURE)*/
                & ((event->state & GDK_MOD1_MASK)
                       ? ~stitch_project::OPTIMIZE_BACKLIGHT_BLACK
                       : ~0),
            &error, &progress))
  ...
  if (ui_mode == screen_editing) {
    ...
    if (k == 'S') {
      if (rparams.collection_quality == render_parameters::fast_collection) {
        rparams.collection_quality =
            render_parameters::simple_screen_collection;
    ...
    if (k == 'l' && !(event->state & GDK_CONTROL_MASK)) {
      if (rparams.dye_balance == render_parameters::dye_balance_none)
        rparams.dye_balance =
            (render_parameters::
                 dye_balance_t)((int)render_parameters::dye_balance_max - 1);
      else
        rparams.dye_balance =
            (render_parameters::dye_balance_t)((int)rparams.dye_balance - 1);
  ...
  if (ui_mode == solver_editing) {
    if ((k == GDK_KEY_Delete || k == GDK_KEY_BackSpace) &&
      ...
      for (int n = 0; n < current_solver.n_points();)
        if (current_solver.points[n].img.x > sel1x &&
            current_solver.points[n].img.x < sel2x &&
            current_solver.points[n].img.y > sel1y &&
            current_solver.points[n].img.y < sel2y)
    ...
    if ((k == GDK_KEY_Delete || k == GDK_KEY_BackSpace) &&
      ...
      for (int n = 0; n < current_solver.n_points();) {
        ...
        if (current_solver.points[n].img.x > sel1x &&
            current_solver.points[n].img.x < sel2x &&
            current_solver.points[n].img.y > sel1y &&
            current_solver.points[n].img.y < sel2y && dist > thresholt)
          current_solver.remove_point(n);
        else
          n++;
      }
    ...
    if (k == 'i' && (event->state & GDK_CONTROL_MASK)) {
      file_progress_info progress(stdout);
      if (rparams.auto_mix_weights_using_ir(scan, current, sel1x, sel1y, sel2x,
                                            sel2y, &progress)) {
        display_scheduled = true;
        setvals();
      }
    ...
    if (k == 'D' && scan.rgbdata) {
      ...
      detected = detect_regular_screen(scan, current_scr_detect, current_solver,
                                       &dsparams, &progress);
      if (detected.success) {
        ...
        {
          rparams.auto_dark_brightness(scan, current, scan.width / 10,
    ...
    if (k == 'l' && (event->state & GDK_CONTROL_MASK)) {
      ...
      int ymax = std::max(sel1y, sel2y);
      bool ret;
      printf("Auto levels in selection in selection %i %i %i %i old darkpoint "
             "%f old brightness %f\n",
             xmin, ymin, xmax, ymax, rparams.dark_point, rparams.brightness);
      {
        file_progress_info progress(stdout);
        ret = rparams.auto_dark_brightness(scan, current, xmin, ymin, xmax,
                                           ymax, &progress);
      }
      if (ret) {
    ...
    if (k == 'f' && (event->state & GDK_MOD1_MASK)) {
      ...
      fparam.range = 4;
      fparam.flags |=
          finetune_position | finetune_bw | finetune_verbose |
          finetune_emulsion_blur /*| finetune_dufay_strips | finetune_fog*/;
      file_progress_info progress(stdout);
      finetune_result res =
          finetune(rparams, current, scan, {{(coord_t)x, (coord_t)y}}, NULL,
                   fparam, &progress);
      if (res.success) {
    ...
    if (k == 'f' && (event->state & GDK_CONTROL_MASK)) {
      ...
      fparam.flags |= finetune_position | finetune_bw | finetune_verbose |
                      /*finetune_screen_blur*/ finetune_scanner_mtf_sigma |
                      finetune_scanner_mtf_defocus /*| finetune_dufay_strips |
                                                      finetune_fog*/
          ;
      file_progress_info progress(stdout);
      finetune_result res =
      ...
      if (res.success) {
        ...
        rparams.sharpen.scanner_mtf.blur_diameter =
    ...
    if (k == 'F' && (event->state & GDK_CONTROL_MASK)) {
      ...
      fparam.multitile = 3;
      fparam.flags |=
          finetune_position | finetune_verbose | finetune_screen_blur |
          (current.type != Joly ? finetune_strips : 0) | finetune_fog;
      file_progress_info progress(stdout);
      finetune_result res =
          finetune(rparams, current, scan, {{(coord_t)x, (coord_t)y}}, NULL,
                   fparam, &progress);
    ...
    if (k == 'w' && (event->state & GDK_CONTROL_MASK)) {
      ...
      if (rparams.auto_white_balance(scan, current, sel1x, sel1y, sel2x, sel2y,
                                     &progress)) {
        display_scheduled = true;
        setvals();
      }
    ...
    if (k == 's' && (event->state & GDK_CONTROL_MASK)) {
      ...
      // fparam.multitile = 3;
      // fparam.flags |= finetune_position | finetune_verbose /*|
      // finetune_screen_mtf_blur*/ | finetune_emulsion_blur /*|
      // finetune_screen_channel_blurs*/ | finetune_screen_blur |
      // finetune_dufay_strips | finetune_fog | finetune_no_normalize;
      fparam.flags |= finetune_position |
                      finetune_verbose /*| finetune_screen_channel_blurs*/ |
                      finetune_scanner_mtf_sigma |
                      finetune_scanner_mtf_defocus |
                      (current.type != Joly ? finetune_strips : 0) |
                      finetune_fog | finetune_no_normalize |
                      finetune_simulate_infrared /*| finetune_sharpening*/;
      // fparam.range = 4;
      fparam.range = 8;
    ...
    if (k == 's' && (event->state & GDK_MOD1_MASK)) {
      ...
      fparam.multitile = 3;
      fparam.flags |=
          finetune_position | finetune_verbose /*| finetune_screen_mtf_blur*/ |
          finetune_emulsion_blur /*| finetune_screen_channel_blurs*/ |
          finetune_screen_blur | (current.type != Joly ? finetune_strips : 0) |
          finetune_fog | finetune_no_normalize;
      fparam.range = 4;
      file_progress_info progress(stdout);
      finetune_result res = finetune(rparams, current, scan, tune_points,
                                     &tune_results, fparam, &progress);
      if (res.success) {
        rparams.screen_blur_radius = res.screen_blur_radius;
        rparams.red_strip_width = res.red_strip_width;
        rparams.green_strip_width = res.green_strip_width;
  ...
  if (ui_mode == screen_editing || ui_mode == solver_editing ||
    ...
    if ((event->state & GDK_CONTROL_MASK) && scan.stitch && k >= '1' &&
        k <= '9') {
      int idx = k - '1';
      ...
      if (rparams.tile_adjustments_width != scan.stitch->params.width ||
        ...
        rparams.set_tile_adjustments_dimensions(scan.stitch->params.width,
                                                scan.stitch->params.height);
    ...
    if (k >= '1' && k <= (int)render_type_max -
                             (int)render_type_first_scr_detect + 1 + '1') {
    ...
    if (k == 'P' || k == 'O') {
      ...
      analyze_color_proportions(current_scr_detect, rparams, scan,
                                k == 'O' ? &current : NULL, minx, miny, maxx,
                                maxy, &progress);
    }
  }
...
/* Initialize the GUI.  */
static GtkWidget *initgtk(int *argc, char **argv) {
  GtkBuilder *builder;
  GtkWidget *window;
  GtkWidget *image_viewer;
  ...
  if (!gtk_builder_add_from_file(builder, DATADIR "/colorscreen/gtkgui.glade",
                                 NULL)
#if 0
      && !gtk_builder_add_from_file (builder, filename.c_str (), NULL))
#endif
      && !gtk_builder_add_from_file(builder, "gtkgui.glade", NULL) &&
      !gtk_builder_add_from_file(builder, "../share/colorscreen/gtkgui.glade",
                                 NULL)) {
  ...
  gtk_signal_connect(GTK_OBJECT(window), "destroy", (GtkSignalFunc)destroy,
                     NULL);
  data.save = GTK_WIDGET(gtk_builder_get_object(builder, "save"));
  /*data.bigimage = GTK_IMAGE (gtk_builder_get_object (builder, "bigimage")); */
  data.smallimage = GTK_IMAGE(gtk_builder_get_object(builder, "smallimage"));
  ...
  gtk_image_viewer_set_scroll_region(GTK_IMAGE_VIEWER(image_viewer), 20, 20,
                                     scan.width - 20, scan.height - 20);
  gtk_image_viewer_set_zoom_range(GTK_IMAGE_VIEWER(image_viewer), 0.01, 64);
  gtk_image_viewer_set_scale_and_shift(GTK_IMAGE_VIEWER(image_viewer), 4.0, 4.0,
                                       64, 64);
  ...
  data.scanner_mtf_sigma =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "mtf-sigma"));
  data.saturation =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "saturation"));
  data.presaturation =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "presaturation"));
  data.sharpen_radius =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "sharpen_radius"));
  data.sharpen_amount =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "sharpen_amount"));
  data.y2 = GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "y2"));
  data.brightness =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "brightness"));
  data.scanner_snr =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "scanner_snr"));
  data.RL_iterations =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "RL_iterations"));
  ...
  data.scanner_mtf_scale =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "scanner_mtf_scale"));
  data.dark_point =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "dark_point"));
  data.collection_threshold =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "collection_threshold"));
  data.balance_black =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "balance_black"));
  data.mix_red = GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "mix_red"));
  data.mix_green =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "mix_green"));
  data.mix_blue = GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "mix_blue"));
  data.balance_red =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "balance_red"));
  data.balance_green =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "balance_green"));
  data.balance_blue =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "balance_blue"));
...
static struct scr_to_img_parameters &get_scr_to_img_parameters() {
  current.mesh_trans = current_mesh;
  return current;
}

static inline void init_transformation_data(scr_to_img *trans) {
  trans->set_parameters(get_scr_to_img_parameters(), scan);
}

/* This renders the small preview widget.   */

static void previewrender(GdkPixbuf **pixbuf) {
  guint8 *pixels;
  ...
  *pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, 0, 8, my_xsize, my_ysize);
  ...
  file_progress_info progress(stdout);
  render_preview(scan, get_scr_to_img_parameters(), rparams, pixels, my_xsize,
                 my_ysize, rowstride, &progress);
  cairo_surface_t *surface = cairo_image_surface_create_for_data(
      pixels, CAIRO_FORMAT_RGB24, my_xsize, my_ysize, rowstride);
#if 0
...
static void draw_circle(cairo_surface_t *surface, coord_t bigscale, int xoffset,
                        int yoffset, int width, int height, coord_t x,
                        coord_t y, coord_t r, coord_t g, coord_t b,
                        coord_t radius = 3, luminosity_t opacity = 0.5) {
...
static void draw_line(cairo_surface_t *surface, coord_t bigscale, int xoffset,
                      int yoffset, coord_t x1, coord_t y1, coord_t x2,
                      coord_t y2, coord_t r, coord_t g, coord_t b,
                      coord_t width = 1, luminosity_t opacity = 0.5) {
  cairo_t *cr = cairo_create(surface);
  cairo_translate(cr, -xoffset, -yoffset);
  cairo_scale(cr, bigscale, bigscale);
...
static void draw_rectangle(cairo_surface_t *surface, coord_t bigscale,
                           int xoffset, int yoffset, coord_t x1, coord_t y1,
                           coord_t x2, coord_t y2, coord_t r, coord_t g,
                           coord_t b, coord_t width = 1,
                           luminosity_t opacity = 0.5) {
  cairo_t *cr = cairo_create(surface);
  cairo_translate(cr, -xoffset, -yoffset);
  cairo_scale(cr, bigscale, bigscale);
...
void draw_text(cairo_surface_t *surface, coord_t bigscale, int xoffset,
               int yoffset, char *text, coord_t x, coord_t y, coord_t r,
               coord_t g, coord_t b, coord_t width = 1,
               luminosity_t opacity = 0.5) {
  cairo_t *cr = cairo_create(surface);
  cairo_translate(cr, -xoffset, -yoffset);
  cairo_scale(cr, bigscale, bigscale);
...
static void bigrender(int xoffset, int yoffset, coord_t bigscale,
...

  {
    ...
    if (ui_mode == screen_detection && scr_detect_display_type) {
      rtparam.type = (enum render_type_t)(
          scr_detect_display_type + (int)render_type_first_scr_detect - 1);
      rtparam.color = color_display;
    } else {
      rtparam.type = ui_mode == screen_detection
                         ? render_type_original
                         : (enum render_type_t)display_type;
      rtparam.color = color_display;
    }
    ...
    ret = render_tile(scan, get_scr_to_img_parameters(), current_scr_detect,
                      rparams, rtparam, tile, &progress);
  }

  cairo_surface_t *surface = cairo_image_surface_create_for_data(
      bigpixels, CAIRO_FORMAT_RGB24, pxsize, pysize, bigrowstride);
  draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize,
              current.center.x, current.center.y, 0, 0, 1);
  draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize,
              current.center.x + current.coordinate1.x,
              current.center.y + current.coordinate1.y, 1, 0, 0);
  draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize,
              current.center.x + current.coordinate2.x,
              current.center.y + current.coordinate2.y, 0, 1, 0);

  draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize,
              current.lens_correction.center.x * scan.width,
              current.lens_correction.center.y * scan.height, 1, 0, 1);
    ...
    for (int y = 0; y < detected.smap->height; y++)
      for (int x = 0; x < detected.smap->width; x++) {
        ...
        if (detected.smap->known_p({xx, yy})) {
          ...
          draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize,
                      img.x, img.y, t == solver_parameters::blue,
                      t == solver_parameters::green,
                      t == solver_parameters::red, 1);
        }
      }
  ...
  if (ui_mode == color_profiling && color_optimizer_points.size()) {
    ...
    for (size_t i = 0; i < color_optimizer_points.size(); i++) {
      ...
      if (!scan.stitch)
        p = map.to_img(
            {color_optimizer_points[i].x, color_optimizer_points[i].y});
      else {
        p = scan.stitch->common_scr_to_img.scr_to_final(
            {color_optimizer_points[i].x, color_optimizer_points[i].y});
        p.x -= scan.xmin;
        p.y -= scan.ymin;
      }
      ...
      if (color_optimizer_match.size() > i) {
        ...
        draw_text(surface, bigscale, xoffset, yoffset, buf, p.x + 27 / bigscale,
                  p.y + 2 / bigscale, 0, 0, 0, 1);
        draw_text(surface, bigscale, xoffset, yoffset, buf, p.x + 25 / bigscale,
                  p.y, 1, 1, 1, 1);
      }
      draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize, p.x, p.y,
                  1, 1, 1, 23 / bigscale, 1);
      draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize, p.x, p.y,
                  c2.blue, c2.green, c2.red, 20 / bigscale, 1);
      draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize, p.x, p.y,
                  c1.blue, c1.green, c1.red, 10 / bigscale, 1);
    }
  }
  ...
  if (current_solver.n_points() && ui_mode == solver_editing) {
    ...
    for (int i = 0; i < current_solver.n_points(); i++) {
      ...
      /* If we do only 1D solving, provide correct y coordinate to minimize
       * distance.  */
      if (screen_with_vertical_strips_p(current.type)) {
      ...
      draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize, xi, yi,
                  color.blue, color.green, color.red);
      draw_circle(surface, bigscale, xoffset, yoffset, pxsize, pysize, p.x, p.y,
                  3 * color.blue / 4, 3 * color.green / 4, 3 * color.red / 4);

      coord_t patch_diam = sqrt(current.coordinate1.x * current.coordinate1.x +
                                current.coordinate1.y * current.coordinate1.y) /
                           2;
      ...
      if (bad)
        draw_line(surface, bigscale, xoffset, yoffset, xi, yi, xi + xd, yi + yd,
                  0, 0, 1, 16);
      draw_line(surface, bigscale, xoffset, yoffset, xi, yi, xi + xd, yi + yd,
                color.blue, color.green, color.red, 6);
      draw_line(surface, bigscale, xoffset, yoffset, xi + xd, yi + yd,
                xi + xd * 0.7 + yd * 0.2, yi + yd * 0.7 - xd * 0.2, color.blue,
                color.green, color.red, 6);
      draw_line(surface, bigscale, xoffset, yoffset, xi + xd, yi + yd,
                xi + xd * 0.7 - yd * 0.2, yi + yd * 0.7 + xd * 0.2, color.blue,
                color.green, color.red, 6);
      draw_line(surface, bigscale, xoffset, yoffset, xi, yi, xi + xd, yi + yd,
  ...
  if (ui_mode == solver_editing) {
    if (sel1x != sel2x || sel1y != sel2y)
      draw_rectangle(surface, bigscale, xoffset, yoffset, sel1x, sel1y, sel2x,
                     sel2y, 1, 1, 1, 6);
  }

  cairo_surface_destroy(surface);
}
static void display_preview() {
...
G_MODULE_EXPORT void cb_redraw(GtkButton *button, Data *data) {
  if (!initialized)
    return;
  getvals();
  display();
...
G_MODULE_EXPORT void cb_press_small(GtkImage *image, GdkEventButton *event,
                                    Data *data) {
  getvals();
  if (event->button == 1) {
    offsetx = 8 + (event->x) * scan.width * bigscale /
                      gdk_pixbuf_get_width(data->smallpixbuf);
    offsety = 8 + (event->y) * scan.height * bigscale /
...
G_MODULE_EXPORT void cb_press(GtkImage *image, GdkEventButton *event,
  ...
  if (ui_mode == color_profiling) {
    ...
    if (!scan.stitch)
      ...
      screen = scan.stitch->common_scr_to_img.final_to_scr(
          {x + scan.xmin, y + scan.ymin});
    if (event->button == 1) {
      ...
      ;
    {
      ...
      for (int i = 0; i < color_optimizer_points.size(); i++) {
        coord_t dist = sqrt((color_optimizer_points[i].x - screen.x) *
                                (color_optimizer_points[i].x - screen.x) +
                            (color_optimizer_points[i].y - screen.y) *
                                (color_optimizer_points[i].y - screen.y));
        if (dist < delta) {
          best = i;
          delta = dist;
  ...
  if (ui_mode == screen_detection) {
    ...
    if (current.type != Random) {
      struct solver_parameters::point_location *points =
          solver_parameters::get_point_locations(current.type, &npoints);
      for (int i = 0; i < npoints; i++) {
        coord_t qscreenx = pscreenx + points[i].x;
        coord_t qscreeny = pscreeny + points[i].y;
        if ((screenx - rscreenx) * (screenx - rscreenx) +
                (screeny - rscreeny) * (screeny - rscreeny) >
            (screenx - qscreenx) * (screenx - qscreenx) +
                (screeny - qscreeny) * (screeny - qscreeny)) {
          rscreenx = qscreenx;
          rscreeny = qscreeny;
    ...
    if (event->button == 3) {
      ...
      for (n = 0; n < current_solver.n_points(); n++) {
        double dist = sqrt((current_solver.points[n].img.x - x) *
                               (current_solver.points[n].img.x - x) +
                           (current_solver.points[n].img.y - y) *
                               (current_solver.points[n].img.y - y));
    ...
    if (event->button == 1 && setcenter) {
      point_t newcenter = {(event->x + shift_x) / scale_x,
                           (event->y + shift_y) / scale_y};
      setcenter = false;
...
void handle_drag(int x, int y, int button) {
  if (ui_mode == screen_detection)
    return;
  gdouble scale_x, scale_y;
  gint shift_x, shift_y;
...
G_MODULE_EXPORT void cb_release(GtkImage *image, GdkEventButton *event,
                                Data *data2) {
  ...
  if (ui_mode == solver_editing && event->button == 1) {
    ...
    if (finetuning && x == xpress && y == ypress) {
      ...
      finetune_result res =
          finetune(rparams, current, scan, {{(coord_t)x, (coord_t)y}}, NULL,
                   fparam, &progress);
      if (res.success) {
        current_solver.add_point(res.solver_point_img_location,
    ...
    if (x == xpress && y == ypress) {
      ...
      if (scale_x < desired_zoom && event->button == 1) {
        ...
        display_type = 0;
        gtk_image_viewer_get_scale_and_shift(
            GTK_IMAGE_VIEWER(data.image_viewer), &saved_scale_x, &saved_scale_y,
            &saved_shift_x, &saved_shift_y);
        gtk_image_viewer_set_scale_and_shift(
            GTK_IMAGE_VIEWER(data.image_viewer), desired_zoom, desired_zoom,
            desired_zoom * x - event->x, desired_zoom * y - event->y);
        return;
      }
      coord_t screenx, screeny;
      coord_t pscreenx, pscreeny;
...
G_MODULE_EXPORT void cb_drag(GtkImage *image, GdkEventMotion *event,
                             Data *data2) {
...
G_MODULE_EXPORT void cb_save(GtkButton *button, Data *data) {
  ...
  if (!save_csp(out, &current, scan.has_rgb() ? &current_scr_detect : NULL,
                &rparams, &current_solver)) {
...
int main(int argc, char **argv) {
  ...
  if (argc != 2 && argc != 3) {
    fprintf(stderr,
            "Invocation: %s <scan> <csp (optional)>]\n\n"
  ...
  const char *error;
  if (in && !load_csp(in, &current, &current_scr_detect, &rparams,
                      &current_solver, &error)) {
  ...
  current.mesh_trans = current_mesh;
  // save_csp (stdout, &current, scan.rgbdata ? &current_scr_detect : NULL,
  // &rparams, &current_solver);
  ...
  gtk_image_viewer_set_scale_and_shift(
      GTK_IMAGE_VIEWER((GtkImageViewer *)data.image_viewer), scale, scale, 0,
      0);
  gtk_image_viewer_redraw((GtkImageViewer *)data.image_viewer, 1);
  ...
  while (true) {
    if (ui_mode == color_profiling &&
      ...
      file_progress_info progress(stdout);
      optimize_color_model_colors(&current, scan, rparams,
                                  color_optimizer_points,
                                  &color_optimizer_match, &progress);
    }
