...
{
  return parse_enum<scr_type,
		    scr_names,
		    (int)max_scr_type> (model, "Unkonwn screen type:%s\n");
}
...
#pragma omp parallel for default (none) collapse(2) schedule (dynamic) shared (xsteps,ysteps,rparam,scan,flags,border,progress,param,orig_tiff_base,simulated_tiff_base,diff_tiff_base,results,multitile)
  ...
  if (flags & (finetune_screen_mtf_blur | finetune_screen_blur | finetune_screen_channel_blurs | finetune_emulsion_blur))
    {
	  ...
	  if (results[y * xsteps + x].success)
	    {
	      ...
	      for (int i = 0; i < 4 ; i++)
	        mtf_hist[i].pre_account (results[y * xsteps + x].screen_mtf_blur[i]);
	    }
	  ...
	  if (results[y * xsteps + x].success)
	    {
	      ...
	      for (int i = 0; i < 4 ; i++)
	        mtf_hist[i].account (results[y * xsteps + x].screen_mtf_blur[i]);
	    }
      hist.finalize ();
	  ...
	  for (int y = 0; y < ysteps; y++)
	    {
	      for (int x = 0; x < xsteps; x++)
		if (results[y * xsteps + x].success)
		  printf ("  %6.3f,%6.3f,%6.3f,%6.3f", results[y * xsteps + x].screen_mtf_blur[0], results[y * xsteps + x].screen_mtf_blur[1], results[y * xsteps + x].screen_mtf_blur[2], results[y * xsteps + x].screen_mtf_blur[3]);
		else
		  printf ("  ------,------,------,------");
	      printf ("\n");
	    }
      ...
      if (screen_blur_tiff_name)
	{
	  ...
	  for (int y = 0; y < ysteps; y++)
		...
		if (!results[y * xsteps + x].success)
		  ...
		  {
		    int vr = std::min (results[y * xsteps + x].screen_mtf_blur[0] / 10 * 65535, (coord_t)65535);
		    int vg = std::min (results[y * xsteps + x].screen_mtf_blur[1] / 10 * 65535, (coord_t)65535);
		    int vb = std::min (results[y * xsteps + x].screen_mtf_blur[2] / 10 * 65535, (coord_t)65535);
...
stitch(int argc, char **argv)
{
  ...
  for (int i = 0; i < argc; i++)
	...
	prj->params.max_contrast = flt;
      else if (parse_float_param (argc, argv, &i, "max-unknown-screen-range", flt, 0, 100))
	...
	prj->params.max_overlap_percentage = atoi (argv[i]);
      else if (parse_int_param (argc, argv, &i, "ncols", ncols, 0, stitching_params::max_dim))
	;
      else if (parse_int_param (argc, argv, &i, "num-control-points", prj->params.num_control_points, 0, stitching_params::max_dim))
	;
      else if (parse_float_param (argc, argv, &i, "min-screen-percentage", flt, 0, 100))
	...
	prj->params.hfov = flt;
      else if (parse_float_param (argc, argv, &i, "max-avg-distance", flt, 0, 100000))
