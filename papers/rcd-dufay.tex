...
\begin{tikzpicture}[scale=0.8, every node/.style={font=\tiny}]
  ...
  \begin{scope}[yshift=2.5cm]
      \node[anchor=west, font=\small\bfseries] at (-1, 1.2) {A) Filter Centered on Red ($s=3$)};
      ...
      \node[pixel, fill=cfagreen!60] (p4) at (5.6, 0) {$x+1$\\(G)};
      \node[pixel, fill=cfared!60]   (p5) at (7.0, 0) {$x+2$\\(R)};
      \node[pixel, fill=cfablue!60]  (p6) at (8.4, 0) {$x+3$\\(B)};
      ...
      \node[anchor=west, align=left] at (9.5, 0) {Non-dominant sum:\\$+1G - 1B - 1G + 1B = 0$\\(Balanced Laplacian)};
  \end{scope}
  
  \begin{scope}[yshift=0cm]
      \node[anchor=west, font=\small\bfseries] at (-1, 1.2) {B) Filter Centered on Green ($s=0$)};
      ...
      \node[anchor=west, align=left, text=red] at (9.5, 0) {Mixed sequence:\\$+1R - 3B - 1R + 6G - 1R - 3B + 1R \neq 0$\\(No cancellation)};
  \end{scope}
\end{tikzpicture}
\end{center}
