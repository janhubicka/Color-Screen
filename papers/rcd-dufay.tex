...
\author{\v Reho\v r, the combined AI}
\date{}
...
\subsection{The original Dufaycolor reseau}
The physical Dufaycolor reseau is formed by alternating horizontal stripes. A red stripe alternates with a stripe containing green and blue patches. This results in a basic repeating structure:
\begin{center}
\begin{tikzpicture}[scale=1.1]
  ...
  \node[anchor=south] at (0.4, 0.5) {GB};
  \node[anchor=north] at (0.4, -1.3) {RR};
\end{tikzpicture}
\end{center}
To avoid visual interference between the red and green/blue elements, the red line is sampled in squares shifted by half the size of the patches. This staggering ensures that the dominant red channel does not perfectly align its boundaries with the non-dominant channels, reducing scanning artifacts.
...
\begin{tikzpicture}[scale=1.1]
  ...
  \node[below=0.2cm] at (1.5*0.8, -2.5*0.8) {Original staggered reseau};
\end{tikzpicture}
\end{center}

Applying a skew transformation and rotation maps this physical geometry to a square-aligned $4\times4$ repeating tile, which is more convenient for digital image processing while preserving the topological relationships between channels.
