...
- **Complete stable-key migrations**: Screen, Geometry, Image Layer, Tiles, Digital Capture, Contact Copy, and Color are complete. `addCorrelatedRGBParameter()` treats its key as a base namespace and assigns `.red`, `.green`, and `.blue` to the saved channels; its Link channels checkbox stays unkeyed presentation state. Color likewise keeps neutral/auto-level actions, chart display selectors, gamut references, and tone-curve coordinate view unkeyed because they do not independently save `ParameterState`.
