...
- **`GeometrySolverController`**: Owns the persistent geometry-solver QThread/worker and replaceable TaskQueue transport. It returns request identity and TaskQueue publication authority to `MainWindow`, which retains geometry-fit provenance, stale-input checks, undo publication, and user-facing failure policy.
