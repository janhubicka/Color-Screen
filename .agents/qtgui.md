...
- **`MainWindow`**: One complete image document. It owns the scan, parameters, undo stack, image/navigation widgets, panels, task queues, workers, a document progress controller, and a unique recovery directory. Mutable document state must never be shared between different `MainWindow` instances.
- **`DocumentProgressController`**: Owns one document's transient status row, dedicated long-task rows, delayed visibility, task switching, periodic status formatting, and cooperative Cancel/Stop presentation. `MainWindow` supplies document/workspace policy such as focus restoration and workspace visibility notifications.
- **`FileRenderController`**: Owns accepted render-to-file jobs after `MainWindow` has chosen the output path and snapshotted the accepted RenderDialog/document settings. Render Settings must restate the already chosen
output contract (destination, format/overwrite, document crop, resolved output
coordinate plane, and export-sharpening policy) rather than making users remember
what they selected in the native file chooser. This summary is presentation
state only; the native save dialog remains authoritative for overwrite
confirmation. It tracks every active file-render job, runs the background work, removes incomplete cancelled/failed output, joins active workers during document teardown, and returns ordinary completion to MainWindow for user-facing status/error reporting.
- **`BackgroundThreadRegistry`**: Tracks ad-hoc document QThreads whose workers publish incremental results (currently misregistered-point and adaptive-sharpening analysis). During document teardown it requests interruption/quit and joins them while servicing only MetaCall events targeted at the owning document, so a worker blocked in a `BlockingQueuedConnection` can return without dispatching unrelated UI events.
...
- **`applyChange`**: Behind the scenes, the helper methods call `applyChange`, which triggers a global state update and UI refresh. Stateful helpers accept an optional stable, untranslated `parameterKey`; use it for new or actively maintained controls so Undo merging and Reset/default metadata do not depend on the visible label. Keys identify the complete processing role, not just the caption: controls in different stages that intentionally share text (for example the Screen panel's pre- and post-demosaic `Strength` controls) must have different keys. If one physical editor changes logical target with selection, the key must include that target too; `addSliderParameter()` accepts a `ParameterKeyGetter` for this case, and Tiles resolves the shared Exposure/Dark point editors to `tiles.<x>.<y>.*` rather than using one key for every tile. Dynamic-key numeric editors may also opt into the standard Reset presentation when their getter can derive the current target's default from a fresh `ParameterState`. The Reset button's key/default/modified metadata must retarget on every refresh, and Reset itself remains a separate undo gesture. Tiles Exposure/Dark point are the reference implementation. The Screen panel provides a complete example under `screen.*`, Geometry follows under `geometry.*`, Image Layer uses `image_layer.*`, and Tiles uses coordinate-specific `tiles.*` keys. Do not assign keys to panel-local operation/presentation toggles merely because they use a `ParameterPanel` helper; Geometry's Auto fit/nonlinear-request toggles, Image Layer's Set-by-area/channel buttons, and the current-tile selector are intentionally unkeyed because they are not saved `ParameterState`. Keyed numeric helpers can opt into the standard default presentation: the label becomes semibold and a small Reset control appears only while the value differs from a fresh `ParameterState` default. Reset remains a separate undo gesture. Those numeric rows also share one centralized left-button double-click Reset gesture on the form label and numeric value editor; it is active only while the row is modified and enabled, so never add panel-specific double-click handlers for the same behavior. Keyed saved enums and ordinary checkboxes may opt into the same fresh-state default presentation when their default has a clear non-destructive meaning. Current checkbox examples are Contact Copy simulation (default off/bypassed while retaining its configured calibration), Sharpness Use measured MTF (default fitted model), Geometry final mirror, and Image Layer Use simulated RGB image layer (default native IR/grayscale source). Do not add saved-default Reset to panel-local Auto fit/nonlinear/focus-analysis options. Preserve explicit sentinel values such as `0 = not configured/process default`: when a sentinel lies below the ordinary numeric interval, pass it as the helper's separate special minimum instead of clamping it to the first explicit value. Screen geometry follows the same rule: fresh `scr_to_img_parameters` have zero coordinate vectors, meaning geometry has not been established. `screen_has_regular_geometry_p(type)` says only that the selected process has a regular lattice; code that consumes screen/final coordinates must use `screen_geometry_configured_p(params)` (and still handle `scr_to_img::set_parameters()` failure). Detection/discovery code is the intentional exception because it establishes the mapping.

- **Complete stable-key migrations**: Screen, Geometry, Image Layer, Tiles, Digital Capture, Contact Copy, Color, and Sharpness are complete. `addCorrelatedRGBParameter()` treats its key as a base namespace and assigns `.red`, `.green`, and `.blue` to the saved channels; its Link channels checkbox stays unkeyed presentation state. Color likewise keeps neutral/auto-level actions, chart display selectors, gamut references, and tone-curve coordinate view unkeyed because they do not independently save `ParameterState`. Sharpness uses `sharpness.*` for saved MTF/deconvolution controls and includes the measurement row target in `sharpness.measurements.<index>.*`; chart presentation, measurement navigation/actions, and focus-analysis setup remain deliberately unkeyed.
...
Workers must periodically call `m_progress->cancel_requested()` and exit
gracefully when it returns true. `progress_info::cancelled()` reports that a
worker has already acknowledged cancellation; it is not the right polling API
for observing a fresh request. Treat `progress_info::pool_cancel()` as an
immediate *publication* veto on the GUI/owner side: once cancellation is
requested, a racing successful or incremental completion must not be applied to
document or presentation state.
