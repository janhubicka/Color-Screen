...
- **`DocumentProgressController`**: Owns one document's transient status row, dedicated long-task rows, delayed visibility, task switching, periodic status formatting, and cooperative Cancel/Stop presentation. `MainWindow` supplies document/workspace policy such as focus restoration, workspace visibility notifications, and confirmation before cancelling a file render.
